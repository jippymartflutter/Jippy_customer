...
class SplashController extends GetxController {
  ...
  Future<void> redirectScreen() async {
    ...
    if (savedLocation != null && savedLocation['latitude'] != null && savedLocation['longitude'] != null) {
      ...
      print('[SPLASH] Reading location from LOCAL STORAGE (GetStorage): lat=${savedLocation['latitude']}, lng=${savedLocation['longitude']}');
    ...
    if (Preferences.getBoolean(Preferences.isFinishOnBoardingKey) == false) {
      print('[SPLASH] Onboarding not finished, but skipping for APK size optimization');
    ...
    print('[SPLASH] AuthHelper.getCurrentUser result: ${firebaseUser?.uid ?? "null"}');
    ...
    if (firebaseUser != null) {
      ...
      print('[SPLASH] getUserProfile result: ${userDoc != null ? "SUCCESS" : "NULL"}');
      ...
      if (userDoc != null) {
        ...
        print('[DEBUG] User role: "${userModel.role}", Expected: "${Constant.userRoleCustomer}"');
        print('[DEBUG] User active status: ${userModel.active}');
        print('[DEBUG] Constant.userModel set: ${Constant.userModel?.toJson()}');
        ...
        if (userModel.role == Constant.userRoleCustomer) {
          if (userModel.active == true) {
            ...
            if (userModel.shippingAddress != null && userModel.shippingAddress!.isNotEmpty) {
              ...
              print('[DEBUG] About to check/migrate Firestore address to local storage');
              if (localLocation == null && Constant.selectedLocation.location != null) {
                ...
                print('[LOCATION] Migrated location from FIRESTORE to LOCAL STORAGE (GetStorage): '
              ...
              print('[SPLASH] Reading location from FIRESTORE: lat=${Constant.selectedLocation.location?.latitude}, lng=${Constant.selectedLocation.location?.longitude}');
              // DashBoardController already registered in main.dart
              Get.offAll(const DashBoardScreen());
            } else {
              print('[SPLASH] No shipping address in Firestore, navigating to LocationPermissionScreen');
          ...
          print('[SPLASH] User role is not customer, navigating to LoginScreen');
          await FirebaseAuth.instance.signOut();
          FireStoreUtils.backendUserId = null;
          await Preferences.setBoolean('isOtpVerified', false);
          Get.offAll(const LoginScreen());
    ...
    // Get.offAll(const LoginScreen());
        ...
        if (apiToken != null && apiToken.isNotEmpty) {
          print('[SPLASH] Found API token but no Firestore profile, attempting to restore session...');
          try {
            ...
            if (response.statusCode == 200 && response.data['user'] != null) {
              ...
              print('[SPLASH] Set backendUserId to: $userId (from backend session)');
              // DashBoardController already registered in main.dart
              Get.offAll(const DashBoardScreen());
              return;
            }
