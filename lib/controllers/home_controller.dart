...
class HomeController extends GetxController {
  ...
  void startBannerTimer() {
    ...
    _bannerTimer = Timer.periodic(const Duration(seconds: 3), (timer) {
      ...
      if (!Get.isRegistered<HomeController>() || !pageController.value.hasClients) {
        timer.cancel();
        return;
      }
  ...
  Future<void> _loadBanners() async {
    ...
    print('[BANNER_LOADING] Current customer zone - ID: $currentZoneId, Title: $currentZoneTitle');
    
    await Future.wait([
      FireStoreUtils.getHomeTopBanner().then((value) {
        bannerModel.value = value;
        print('[BANNER_LOADING] Top banners loaded: ${value.length}');
        print('[BANNER_LOADING] Top banner details: ${value.map((b) => '${b.title} (Zone: ${b.zoneId})').join(', ')}');
      }),
      FireStoreUtils.getHomeBottomBanner().then((value) {
        ...
        print('[BANNER_LOADING] Bottom banner details: ${value.map((b) => '${b.title} (Zone: ${b.zoneId})').join(', ')}');
      }),
    ]);
    
    print('[BANNER_LOADING] Total banners loaded - Top: ${bannerModel.length}, Bottom: ${bannerBottomModel.length}');
  }
  ...
  Future<void> _ensureUserModelIsLoaded() async {
    ...
    for (int attempt = 1; attempt <= 5; attempt++) {
      ...
      if (Constant.userModel != null) {
        print('[DEBUG] User model loaded successfully: ${Constant.userModel!.id}');
        return;
      }
      
      // Wait before retry
      if (attempt < 5) {
        print('[DEBUG] User model not loaded yet, waiting 200ms before retry...');
        await Future.delayed(Duration(milliseconds: 200));
      }
    }
    
    print('[DEBUG] User model not loaded after 5 attempts, trying to load fresh...');
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        if (userModel != null) {
          ...
          print('[DEBUG] User model loaded fresh from Firestore: ${userModel.id}');
          return;
        }
      }
    } catch (e) {
  ...
  Future<void> _ensureUserLocationIsSet() async {
    ...
    print('[DEBUG] Current location: ${Constant.selectedLocation.location?.latitude}, ${Constant.selectedLocation.location?.longitude}');
    ...
    for (int attempt = 1; attempt <= 3; attempt++) {
      ...
      if (Constant.userModel != null && 
        ...
        if (defaultAddress.location?.latitude != null && defaultAddress.location?.longitude != null) {
          Constant.selectedLocation = defaultAddress;
          print('[DEBUG] Location set from user model (attempt $attempt): ${defaultAddress.location?.latitude}, ${defaultAddress.location?.longitude}');
          return;
        }
      }
      ...
      try {
        ...
        if (savedLocation != null && savedLocation['latitude'] != null && savedLocation['longitude'] != null) {
          ...
          if (savedAddress.isEmpty || savedLocality.isEmpty) {
            try {
              ...
              final gpsCacheInfo = await GpsLocationService.getCachedAddressInfo();
          ...
          print('[DEBUG] Location set from local storage (attempt $attempt): ${savedLocation['latitude']}, ${savedLocation['longitude']} - $savedAddress');
          return;
        }
      } catch (e) {
        print('[DEBUG] Error reading location from local storage (attempt $attempt): $e');
      }
      
      // **NEW: Try to get location from GPS as fallback**
      try {
        print('[DEBUG] Attempting GPS location detection (attempt $attempt)...');
        final gpsLocation = await GpsLocationService.getLocationForZoneDetection();
        if (gpsLocation != null && gpsLocation['latitude'] != null && gpsLocation['longitude'] != null) {
          ...
          print('[DEBUG] Location set from GPS (attempt $attempt): ${gpsLocation['latitude']}, ${gpsLocation['longitude']} - $fullAddress');
          return;
        }
      } catch (e) {
      ...
      if (Constant.userModel == null) {
        print('[DEBUG] User model not loaded yet, waiting 500ms before retry...');
    ...
    print('[DEBUG] No valid location found after 3 attempts, will use fallback zone');
  ...
  Future<String?> _detectZoneIdForCoordinates(double latitude, double longitude) async {
    try {
      print('[DEBUG] Starting zone detection for coordinates: $latitude, $longitude');
  ...
  String _getLocationNameFromCoordinates(double latitude, double longitude) {
    ...
    // Delhi area
    if (latitude >= 28.6 && latitude <= 28.7 && longitude >= 77.2 && longitude <= 77.3) {
      return 'Delhi, India';
    }
    
    // Bangalore area
    if (latitude >= 12.9 && latitude <= 13.0 && longitude >= 77.6 && longitude <= 77.7) {
      return 'Bangalore, India';
    }
    
    // Chennai area
    if (latitude >= 13.0 && latitude <= 13.1 && longitude >= 80.2 && longitude <= 80.3) {
      return 'Chennai, India';
    }
    
    // Hyderabad area
    if (latitude >= 17.4 && latitude <= 17.5 && longitude >= 78.4 && longitude <= 78.5) {
    ...
    // Kolkata area
    if (latitude >= 22.5 && latitude <= 22.6 && longitude >= 88.3 && longitude <= 88.4) {
      return 'Kolkata, India';
    }
    
    // Ahmedabad area
    if (latitude >= 23.0 && latitude <= 23.1 && longitude >= 72.6 && longitude <= 72.7) {
      return 'Ahmedabad, India';
    }
    
    // Jaipur area
    if (latitude >= 26.9 && latitude <= 27.0 && longitude >= 75.8 && longitude <= 75.9) {
      return 'Jaipur, India';
    }
    
    // Kochi area
    if (latitude >= 9.9 && latitude <= 10.0 && longitude >= 76.2 && longitude <= 76.3) {
      return 'Kochi, India';
    }
    
    // Goa area
    if (latitude >= 15.4 && latitude <= 15.5 && longitude >= 73.8 && longitude <= 73.9) {
      return 'Goa, India';
    }
  ...
  Future<void> _debugLocationSources() async {
    ...
    print('📍 Constant.selectedLocation: ${Constant.selectedLocation.location?.latitude}, ${Constant.selectedLocation.location?.longitude}');
    print('👤 Constant.userModel: ${Constant.userModel?.id ?? "NULL"}');
    
    if (Constant.userModel?.shippingAddress != null) {
      print('🏠 User shipping addresses: ${Constant.userModel!.shippingAddress!.length}');
      for (int i = 0; i < Constant.userModel!.shippingAddress!.length; i++) {
        final addr = Constant.userModel!.shippingAddress![i];
        print('   Address $i: ${addr.addressAs} - ${addr.location?.latitude}, ${addr.location?.longitude} (default: ${addr.isDefault})');
      }
    } else {
    ...
    try {
      final box = GetStorage();
      final savedLocation = box.read('user_location');
      print('💾 Local storage location: ${savedLocation?['latitude']}, ${savedLocation?['longitude']}');
    } catch (e) {
    ...
    try {
      ...
      if (isGpsAvailable) {
        final gpsLocation = await GpsLocationService.getLocationForZoneDetection();
        print('📡 GPS location: ${gpsLocation?['latitude']}, ${gpsLocation?['longitude']}');
      }
    } catch (e) {
  ...
  void logRestaurantDiagnostics() {
    ...
    print('📍 Selected Zone: ${Constant.selectedZone?.name ?? "None"} (ID: ${Constant.selectedZone?.id ?? "None"})');
    print('🌍 User Location: ${Constant.selectedLocation.location?.latitude}, ${Constant.selectedLocation.location?.longitude}');
  ...
  setLoading() async {
    await Future.delayed(Duration(seconds: 1), () async {
      print('[DEBUG] setLoading() - Restaurant count: ${allNearestRestaurant.length}');
      print('[DEBUG] setLoading() - Zone available: ${Constant.isZoneAvailable}');
      print('[DEBUG] setLoading() - Selected zone: ${Constant.selectedZone?.name}');
      
      if (allNearestRestaurant.isEmpty) {
        print('[DEBUG] setLoading() - No restaurants found, extending loading time');
        await Future.delayed(Duration(seconds: 2), () {
          isLoading.value = false;
        });
      } else {
        print('[DEBUG] setLoading() - Restaurants found, setting loading to false');
        isLoading.value = false;
      }
      update();
    });
  ...
  getVendorCategory() async {
    ...
    // but kept for backward compatibility
    print('[DEBUG] getVendorCategory() called - using parallel loading instead');
    await _loadVendorCategories();
  }
  ...
  Future<void> _calculateDistancesInBatches(List<VendorModel> vendors) async {
    ...
    for (int i = 0; i < vendors.length; i += batchSize) {
      final end = (i + batchSize < vendors.length) ? i + batchSize : vendors.length;
  ...
  getZone() async {
    print('[DEBUG] getZone() called - User location: ${Constant.selectedLocation.location?.latitude}, ${Constant.selectedLocation.location?.longitude}');
    await FireStoreUtils.getZone().then((value) async {
      if (value != null) {
        ...
        if (detectedZone != null) {
          print('[DEBUG] User location: ${Constant.selectedLocation.location?.latitude}, ${Constant.selectedLocation.location?.longitude}');
          Constant.selectedZone = detectedZone;
          Constant.isZoneAvailable = true;
          print('[DEBUG] Zone detected: ${detectedZone.name} (${detectedZone.id})');
        } else 
        {
       /*
...
// Try to find any available zone as fallback
  ...
  Future<void> _setFallbackZoneFromFirebase() async {
    try {
      ...
      if (fallbackConfig.exists && fallbackConfig.data() != null) {
        ...
        if (isEnabled && fallbackZoneId != null) {
          ...
          if (fallbackZoneDoc.exists && fallbackZoneDoc.data() != null) {
            ...
            Constant.selectedZone = fallbackZone;
            Constant.isZoneAvailable = false; // User is outside service area
            print('[DEBUG] Using Firebase fallback zone: ${fallbackZone.name} (${fallbackZone.id})');
            print('[DEBUG] User is outside service area - no restaurants will be shown');
      ...
      print('[DEBUG] Firebase fallback config not found, using local fallback...');
  ...
  Future<void> _setLocalFallbackZone() async {
    try {
      ...
      if (fallbackZone != null) {
        Constant.selectedZone = fallbackZone;
        Constant.isZoneAvailable = false; // User is outside service area
        print('[DEBUG] Using local fallback zone: ${fallbackZone.name} (${fallbackZone.id})');
        print('[DEBUG] User is outside service area - no restaurants will be shown');
      ...
      if (cached != null) {
        ...
        if (timestamp != null) {
          ...
          if (now.difference(cacheTime) < Duration(hours: 24)) {
            print('[DEBUG] Using cached fallback zone config: ${cached['zone_id']}');
            return cached;
          }
        }
      }
