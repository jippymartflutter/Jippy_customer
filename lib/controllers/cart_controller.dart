...
import '../widgets/delivery_zone_alert_dialog.dart';
...
class CartController extends GetxController with CrashPreventionMixin, SystemCallOptimizerMixin {
  ...
  Future<void> _initializeAddressWithPriority() async {
    try {
      ...
      if (Constant.userModel != null && 
        ...
        print('🏠 [ADDRESS_PRIORITY] ✅ PRIORITY 1 SUCCESS - Using saved address: ${defaultAddress.address}');
        print('🏠 [ADDRESS_PRIORITY] Address ID: ${defaultAddress.id}');
        print('🏠 [ADDRESS_PRIORITY] Address locality: ${defaultAddress.locality}');
        print('🏠 [ADDRESS_PRIORITY] Address zone: ${defaultAddress.zoneId ?? "NULL"}');
        print('🏠 [ADDRESS_PRIORITY] ⚠️ IGNORING GPS LOCATION - Using saved address only');
        print('🏠 [ADDRESS_PRIORITY] ==========================================');
        return;
      }
      
      print('🏠 [ADDRESS_PRIORITY] ❌ PRIORITY 1 FAILED - No saved addresses found');
      print('🏠 [ADDRESS_PRIORITY] Available addresses: ${Constant.userModel?.shippingAddress?.length ?? 0}');
      
      // PRIORITY 2: Try to get address from home screen (Constant.selectedLocation)
      print('🏠 [ADDRESS_PRIORITY] PRIORITY 2: Attempting to get address from home screen...');
      final homeScreenAddress = await _getCurrentLocationAddress();
      
      if (homeScreenAddress != null) {
        selectedAddress.value = homeScreenAddress;
        print('🏠 [ADDRESS_PRIORITY] ✅ PRIORITY 2 SUCCESS - Using home screen address: ${homeScreenAddress.address}');
        print('🏠 [ADDRESS_PRIORITY] Home screen address locality: ${homeScreenAddress.locality}');
        print('🏠 [ADDRESS_PRIORITY] Home screen coordinates: lat=${homeScreenAddress.location?.latitude}, lng=${homeScreenAddress.location?.longitude}');
        return;
      }
      
      print('🏠 [ADDRESS_PRIORITY] ❌ PRIORITY 2 FAILED - Could not get home screen address');
      
      // PRIORITY 3: BLOCK ORDER - NO FALLBACK ZONES
      print('🏠 [ADDRESS_PRIORITY] ❌ PRIORITY 3 - BLOCKING ORDER - No valid address available');
      print('🏠 [ADDRESS_PRIORITY] ==========================================');
  ...
  Future<ShippingAddress?> _getCurrentLocationAddress() async {
    try {
      print('📍 [HOME_SCREEN_ADDRESS] Attempting to get address from home screen...');
      ...
      if (Constant.selectedLocation.location?.latitude != null && 
        ...
        if (lat >= 6.0 && lat <= 37.0 && lng >= 68.0 && lng <= 97.0) {
          ...
          if (address.isEmpty || locality.isEmpty || 
              address == 'Current Location' || locality == 'Current Location' ||
              address.contains('Current Location') || locality.contains('Current Location')) {
            print('📍 [HOME_SCREEN_ADDRESS] ❌ Home screen address is invalid or incomplete');
            print('📍 [HOME_SCREEN_ADDRESS] Address: "$address"');
            print('📍 [HOME_SCREEN_ADDRESS] Locality: "$locality"');
            return null;
          }
          
          print('📍 [HOME_SCREEN_ADDRESS] ✅ Successfully got address from home screen');
          print('📍 [HOME_SCREEN_ADDRESS] Address: "$address"');
          print('📍 [HOME_SCREEN_ADDRESS] Locality: "$locality"');
          ...
          print('📍 [HOME_SCREEN_ADDRESS] Detected zone ID: ${detectedZoneId ?? "NULL"}');
          
          return ShippingAddress(
            id: 'home_screen_address_${DateTime.now().millisecondsSinceEpoch}',
            addressAs: Constant.selectedLocation.addressAs ?? 'Home Screen Address',
            address: address,
            locality: locality,
      ...
      print('📍 [HOME_SCREEN_ADDRESS] ❌ Could not get valid address from home screen');
      return null;
  ...
  Future<String?> _reverseGeocodeCoordinates(double lat, double lng) async {
    try {
      ...
      print('📍 [REVERSE_GEOCODE] ❌ DEPRECATED - Not creating coordinate-based addresses');
      return null;
    } catch (e) {
  ...
  void _showAddressRequiredAlert() {
    Get.dialog(
      AlertDialog(
        ...
        content: Text('Please add a delivery address to continue with your order.'.tr),
        actions: [
          TextButton(
            onPressed: () {
              Get.back();
  ...
  Future<String?> _detectZoneIdForCoordinates(double latitude, double longitude) async {
    try {
      print('🗺️ [ZONE_DETECTION] Starting zone detection for coordinates: $latitude, $longitude');
      ...
      for (ZoneModel zone in zones) {
        if (zone.area != null && zone.area!.isNotEmpty) {
          print('🗺️ [ZONE_DETECTION] Checking zone: ${zone.name} (${zone.id})');
          ...
          if (Constant.isPointInPolygon(
            ...
            print('🗺️ [ZONE_DETECTION] ✅ Zone detected: ${zone.name} (${zone.id})');
            return zone.id;
          }
        }
      }
  ...
  Future<void> validateUserProfileBulletproof() async {
    ...
    try {
      print('🔒 [BULLETPROOF_PROFILE] ==========================================');
      print('🔒 [BULLETPROOF_PROFILE] VALIDATION STARTED at ${startTime.toIso8601String()}');
      print('🔒 [BULLETPROOF_PROFILE] User ID: ${FireStoreUtils.getCurrentUid()}');
      print('🔒 [BULLETPROOF_PROFILE] Current user model: ${userModel.value.firstName ?? 'NULL'}');
      print('🔒 [BULLETPROOF_PROFILE] Cached user model: ${Constant.userModel?.firstName ?? 'NULL'}');
      ...
      while (user == null && attempts < maxAttempts) {
        attempts++;
        print('🔒 [BULLETPROOF_PROFILE] Attempt $attempts/$maxAttempts at ${DateTime.now().toIso8601String()}');
        
        try {
          // Strategy 1: Try fresh Firestore fetch
          print('🔒 [BULLETPROOF_PROFILE] Strategy 1: Fresh Firestore fetch (10s timeout)');
          final fetchStart = DateTime.now();
          
          user = await FireStoreUtils.getUserProfile(FireStoreUtils.getCurrentUid())
              .timeout(const Duration(seconds: 10));
          
          final fetchDuration = DateTime.now().difference(fetchStart);
          print('🔒 [BULLETPROOF_PROFILE] Firestore fetch completed in ${fetchDuration.inMilliseconds}ms');
          
          if (user != null) {
            print('🔒 [BULLETPROOF_PROFILE] ✅ Fresh Firestore fetch SUCCESSFUL');
            print('🔒 [BULLETPROOF_PROFILE] User data: firstName="${user.firstName}", phone="${user.phoneNumber}", email="${user.email}"');
            break;
          } else {
            print('🔒 [BULLETPROOF_PROFILE] ❌ Fresh Firestore fetch returned NULL');
          }
        } catch (e) {
          ...
          if (attempts == 2 && Constant.userModel != null) {
            print('🔒 [BULLETPROOF_PROFILE] Strategy 2: Using cached user data as fallback');
            print('🔒 [BULLETPROOF_PROFILE] Cached data: firstName="${Constant.userModel!.firstName}", phone="${Constant.userModel!.phoneNumber}"');
            user = Constant.userModel;
            print('🔒 [BULLETPROOF_PROFILE] ✅ Cached user data used as fallback');
            break;
          }
          
          // Strategy 3: Wait and retry for network issues
          if (attempts < maxAttempts) {
            print('🔒 [BULLETPROOF_PROFILE] Strategy 3: Waiting 2 seconds before retry...');
            await Future.delayed(const Duration(seconds: 2));
            print('🔒 [BULLETPROOF_PROFILE] Wait completed, proceeding to next attempt');
          }
        }
      }
      ...
      if (user == null) {
        ...
        print('🔒 [BULLETPROOF_PROFILE] Total duration: ${totalDuration.inMilliseconds}ms');
        ...
        ShowToastDialog.showToast("Unable to verify profile. Please check your internet connection and try again.".tr);
        return;
      }
      
      print('🔒 [BULLETPROOF_PROFILE] ✅ User data retrieved successfully');
      print('🔒 [BULLETPROOF_PROFILE] Raw data - firstName: "${user.firstName}", phoneNumber: "${user.phoneNumber}", email: "${user.email}"');
      ...
      print('🔒 [BULLETPROOF_PROFILE] - First Name: ${hasFirstName ? "✅ VALID" : "❌ INVALID"} (value: "${user.firstName}", length: ${user.firstName?.length ?? 0})');
      print('🔒 [BULLETPROOF_PROFILE] - Phone Number: ${hasPhoneNumber ? "✅ VALID" : "❌ INVALID"} (value: "${user.phoneNumber}", length: ${user.phoneNumber?.length ?? 0})');
      print('🔒 [BULLETPROOF_PROFILE] - Email: ${hasEmail ? "✅ VALID" : "❌ INVALID"} (value: "${user.email}", contains @: ${user.email?.contains('@') ?? false}, contains .: ${user.email?.contains('.') ?? false})');
      
      isProfileValid.value = hasFirstName && hasPhoneNumber && hasEmail;
      
      final totalDuration = DateTime.now().difference(startTime);
      print('🔒 [BULLETPROOF_PROFILE] ==========================================');
      print('🔒 [BULLETPROOF_PROFILE] FINAL RESULT: ${isProfileValid.value ? "✅ PROFILE_VALID" : "❌ PROFILE_INVALID"}');
      print('🔒 [BULLETPROOF_PROFILE] Total duration: ${totalDuration.inMilliseconds}ms');
      ...
      if (!isProfileValid.value) {
        print('🔒 [BULLETPROOF_PROFILE] ❌ Profile validation failed - missing required fields');
        ...
        print('🔒 [BULLETPROOF_PROFILE] Missing fields: ${missingFields.join(', ')}');
      }
      ...
      print('🔒 [BULLETPROOF_PROFILE] ==========================================');
      isProfileValid.value = false;
      ShowToastDialog.showToast("Error validating profile. Please try again.".tr);
    } finally {
      isProfileValidating.value = false;
      print('🔒 [BULLETPROOF_PROFILE] Validation completed, isProfileValidating set to false');
    }
  }
  ...
  */
  ...
  Future<bool> validateAndPlaceOrderBulletproof() async {
    ...
    print('🚀 [BULLETPROOF_ORDER] ORDER VALIDATION STARTED at ${startTime.toIso8601String()}');
    print('🚀 [BULLETPROOF_ORDER] Cart items: ${cartItem.length}');
    print('🚀 [BULLETPROOF_ORDER] Total amount: ₹${totalAmount.value}');
    print('🚀 [BULLETPROOF_ORDER] Selected payment: ${selectedPaymentMethod.value}');
    print('🚀 [BULLETPROOF_ORDER] Selected address: ${selectedAddress.value?.address ?? "NULL"}');
    ...
    print('🚀 [BULLETPROOF_ORDER] Profile validation completed in ${profileDuration.inMilliseconds}ms');
    print('🚀 [BULLETPROOF_ORDER] Profile validation result: ${isProfileValid.value ? "✅ VALID" : "❌ INVALID"}');
    ...
    if (!isProfileValid.value) {
      ...
      if (user?.firstName == null || user!.firstName!.trim().isEmpty || user.firstName!.trim().length < 2) {
        missingFields.add("First Name (minimum 2 characters)");
      }
      if (user?.phoneNumber == null || user!.phoneNumber!.trim().isEmpty || user.phoneNumber!.trim().length < 10) {
        missingFields.add("Phone Number (minimum 10 digits)");
      }
      if (user?.email == null || user!.email!.trim().isEmpty || !user.email!.contains('@')) {
      ...
      print('🚀 [BULLETPROOF_ORDER] Missing fields: ${missingFields.join(', ')}');
      print('🚀 [BULLETPROOF_ORDER] Total duration: ${totalDuration.inMilliseconds}ms');
      print('🚀 [BULLETPROOF_ORDER] Final result: ORDER_BLOCKED (PROFILE_INVALID)');
      print('🚀 [BULLETPROOF_ORDER] ==========================================');

      ShowToastDialog.showToast(message);
      return false;
    }
    print('🚀 [BULLETPROOF_ORDER] ✅ STEP 1 PASSED - Profile validation successful');
    ...
    print('🚀 [BULLETPROOF_ORDER] Address validation completed in ${addressDuration.inMilliseconds}ms');
    print('🚀 [BULLETPROOF_ORDER] Address validation result: ${addressValid ? "✅ VALID" : "❌ INVALID"}');
    
    if (!addressValid) {
      final totalDuration = DateTime.now().difference(startTime);
      print('🚀 [BULLETPROOF_ORDER] ❌ STEP 2 FAILED - Address invalid');
      print('🚀 [BULLETPROOF_ORDER] Total duration: ${totalDuration.inMilliseconds}ms');
      print('🚀 [BULLETPROOF_ORDER] Final result: ORDER_BLOCKED (ADDRESS_INVALID)');
      print('🚀 [BULLETPROOF_ORDER] ==========================================');
      return false;
    }
    print('🚀 [BULLETPROOF_ORDER] ✅ STEP 2 PASSED - Address validation successful');

    // STEP 3: MINIMUM ORDER VALIDATION
    print('🚀 [BULLETPROOF_ORDER] STEP 3: Starting minimum order validation...');
    ...
    try {
      ...
      print('🚀 [BULLETPROOF_ORDER] Minimum order validation completed in ${minOrderDuration.inMilliseconds}ms');
      print('🚀 [BULLETPROOF_ORDER] ✅ STEP 3 PASSED - Minimum order validation successful');
    } catch (e) {
      final totalDuration = DateTime.now().difference(startTime);
      print('🚀 [BULLETPROOF_ORDER] ❌ STEP 3 FAILED - Minimum order validation error: $e');
      print('🚀 [BULLETPROOF_ORDER] Total duration: ${totalDuration.inMilliseconds}ms');
      print('🚀 [BULLETPROOF_ORDER] Final result: ORDER_BLOCKED (MIN_ORDER_INVALID)');
      print('🚀 [BULLETPROOF_ORDER] ==========================================');
      return false;
    }
    ...
    final totalDuration = DateTime.now().difference(startTime);
    print('🚀 [BULLETPROOF_ORDER] ==========================================');
    print('🚀 [BULLETPROOF_ORDER] ✅ ALL 3 STEPS PASSED - ORDER VALIDATION SUCCESSFUL');
    print('🚀 [BULLETPROOF_ORDER] Validation breakdown:');
    print('🚀 [BULLETPROOF_ORDER] - Profile validation: ${profileDuration.inMilliseconds}ms');
    print('🚀 [BULLETPROOF_ORDER] - Address validation: ${addressDuration.inMilliseconds}ms');
    print('🚀 [BULLETPROOF_ORDER] - Min order validation: ${DateTime.now().difference(minOrderStartTime).inMilliseconds}ms');
    print('🚀 [BULLETPROOF_ORDER] Total validation duration: ${totalDuration.inMilliseconds}ms');
  ...
  int? getPromotionalItemLimit(String productId, String restaurantId) {
    try {
      ...
      */
      ...
      final limit = PromotionalCacheService.getPromotionalItemLimit(productId, restaurantId);
      
      if (limit != null) {
        print('DEBUG: getPromotionalItemLimit - Found promotional limit: $limit');
      } else {
 ...
 final isAllowed = currentQuantity <= limit;
    ...
    */
    ...
    print('DEBUG: isPromotionalItemQuantityAllowed - Current: $currentQuantity, Allowed: $isAllowed');
  ...
  bool isPaymentReadyToProceed() {
    ...
    final addressValid = selectedAddress.value?.id != null && selectedAddress.value!.id!.isNotEmpty;
  ...
  getCartData() async {
    ...
    cartProvider.cartStream.listen(
        ...
        if (cartItem.isNotEmpty) {
          ...
          if (martItems.isNotEmpty) {
            ...
            print('[VENDOR_LOAD] 🏪 Cart contains mart items, loading mart vendor...');
            try {
              ...
              print('[VENDOR_LOAD] 🔍 Loading mart vendor for vendorID: $vendorId');
              ...
              if (vendorId != null && vendorId.isNotEmpty) {
                ...
                if (martVendor != null) {
                  print('[VENDOR_LOAD] ✅ Found specific mart vendor: ${martVendor.title} (${martVendor.id})');
                } else {
                  print('[VENDOR_LOAD] ⚠️ Specific mart vendor not found, trying default mart vendor...');
                  // Fallback to default mart vendor
                  martVendor = await MartVendorService.getDefaultMartVendor();
                }
              } else {
                print('[VENDOR_LOAD] ⚠️ No vendorID in mart item, trying default mart vendor...');
                // Fallback to default mart vendor
                martVendor = await MartVendorService.getDefaultMartVendor();
              }
              ...
              if (martVendor != null) {
                ...
                vendorModel.value = VendorModel(
                  ...
                  isSelfDelivery: false, // Mart vendors don't have self delivery, use false
                ...
                _updateCacheTime();
                print('[VENDOR_LOAD] ✅ Mart vendor loaded: ${martVendor.title} (${martVendor.id})');
                print('[VENDOR_LOAD]   - Location: (${martVendor.latitude}, ${martVendor.longitude})');
                print('[VENDOR_LOAD]   - Is Self Delivery: false (mart vendors use regular delivery)');
              } else {
            ...
            print('[VENDOR_LOAD] 🍽️ Cart contains restaurant items, loading restaurant vendor...');
            // Use cached vendor data if available
            if (_cachedVendorModel != null && _isCacheValid()) {
              vendorModel.value = _cachedVendorModel!;
              print('[VENDOR_LOAD] ✅ Using cached restaurant vendor: ${vendorModel.value.title}');
            } else {
              await FireStoreUtils.getVendorById(cartItem.first.vendorID.toString())
                  ...
                  if (value != null) {
                    ...
                    _updateCacheTime();
                    print('[VENDOR_LOAD] ✅ Restaurant vendor loaded: ${value.title} (${value.id})');
                  }
                },
              );
            }
    ...
    print('[COUPON_DEBUG] - _cachedCouponList: ${_cachedCouponList?.length ?? 'null'}');
    ...
    if (vendorModel.value.id != null && (!_isCacheValid() || _cachedCouponList == null)) {
      ...
      print('[COUPON_DEBUG] - vendorModel.value.id != null: ${vendorModel.value.id != null}');
      print('[COUPON_DEBUG] - (!_isCacheValid() || _cachedCouponList == null): ${(!_isCacheValid() || _cachedCouponList == null)}');
      
      // Force load coupons if we have a vendor but no coupons loaded yet
      if (vendorModel.value.id != null && _cachedCouponList == null) {
        print('[COUPON_DEBUG] 🔧 Force loading coupons - vendor exists but no cached coupons');
        await _loadCoupons();
      }
    }
  }
  ...
  Future<void> _loadCoupons() async {
    try {
      ...
      print('[COUPON_DEBUG] - Global Coupons (filtered): ${filteredGlobalCoupons.length}');
      ...
      for (int i = 0; i < vendorCoupons.length; i++) {
        ...
        print('[COUPON_DEBUG]   ${i + 1}. ${coupon.code} (${coupon.cType}) - ${coupon.resturantId}');
      }
      ...
      final contextFilteredAllCoupons = CouponFilterService.filterCouponsByContext(
      ...
      final stats = CouponFilterService.getCouponStats(combinedCoupons.cast<CouponModel>());
      print('[COUPON_LOAD] 📊 Coupon Stats: ${stats.toString()}');
      print('[COUPON_LOAD] 🎯 Context: ${_currentContext.value}, Filtered: ${contextFilteredCoupons.length}/${combinedCoupons.length}');
  ...
  Future<void> _loadCouponsWithoutFiltering() async {
    try {
      print('[COUPON_LOAD] 🔄 Loading coupons without filtering as fallback...');
  ...
  bool _isMartItem(CartProductModel item) {
    try {
      ...
      if (item.vendorName != null) {
        ...
        if (vendorName.contains("jippy mart") || 
            vendorName.contains("mart")) {
          return true;
        }
      }
  ...
  bool isMartDeliveryFree() {
    try {
      ...
      if (_martDeliverySettings != null) {
        itemThreshold = (_martDeliverySettings!['item_total_threshold'] as num?)?.toDouble() ?? 199.0;
        freeDeliveryKm = (_martDeliverySettings!['free_delivery_distance_km'] as num?)?.toDouble() ?? 5.0;
      }
      
      final isEligible = subTotal.value >= itemThreshold && totalDistance.value <= freeDeliveryKm;
  ...
  void setContext(String contextType) {
    if (contextType == "mart" || contextType == "restaurant") {
      ...
      print('[COUPON_LOAD] ⚠️ Invalid context type: $contextType. Use "mart" or "restaurant"');
    }
  }
  ...
  void forceLoadCouponsUnconditionally() {
    ...
    print('[COUPON_DEBUG] - vendorModel.value.id: ${vendorModel.value.id}');
    print('[COUPON_DEBUG] - vendorModel.value: ${vendorModel.value.toString()}');
  ...
  calculatePrice() async {
    ...
    if (vendorModel.value.id == null) {
      final martItems = cartItem.where((item) => _isMartItem(item)).toList();
      if (martItems.isNotEmpty) {
        print('[VENDOR_LOAD] 🔧 Fallback: Loading mart vendor in calculatePrice...');
        try {
          // Get the vendorID from the first mart item to load the specific mart vendor
          final firstMartItem = martItems.first;
          final vendorId = firstMartItem.vendorID;
          
          print('[VENDOR_LOAD] 🔧 Fallback: Loading mart vendor for vendorID: $vendorId');
          ...
          if (vendorId != null && vendorId.isNotEmpty) {
            ...
            martVendor = await MartVendorService.getMartVendorById(vendorId);
            if (martVendor != null) {
              print('[VENDOR_LOAD] ✅ Fallback: Found specific mart vendor: ${martVendor.title} (${martVendor.id})');
            } else {
              print('[VENDOR_LOAD] ⚠️ Fallback: Specific mart vendor not found, trying default mart vendor...');
              // Fallback to default mart vendor
              martVendor = await MartVendorService.getDefaultMartVendor();
            }
          } else {
            print('[VENDOR_LOAD] ⚠️ Fallback: No vendorID in mart item, trying default mart vendor...');
            // Fallback to default mart vendor
            martVendor = await MartVendorService.getDefaultMartVendor();
          }
          ...
          if (martVendor != null) {
            ...
            print('[VENDOR_LOAD] ✅ Fallback: Mart vendor loaded: ${martVendor.title} (${martVendor.id})');
          }
        } catch (e) {
    ...
    if (cartItem.isNotEmpty) {
      if (selectedFoodType.value == "Delivery") {
        ...
        print('[DISTANCE_CALC]   - Latitude: ${selectedAddress.value?.location?.latitude}');
        print('[DISTANCE_CALC]   - Longitude: ${selectedAddress.value?.location?.longitude}');
        print('[DISTANCE_CALC]   - Address: ${selectedAddress.value?.address}');
        ...
        print('[DISTANCE_CALC]   - Longitude: ${vendorModel.value.longitude}');
        ...
        if (selectedAddress.value?.location?.latitude != null &&
          ...
          print('[DISTANCE_CALC] ✅ All location data available, calculating distance...');
          ...
          print('[DISTANCE_CALC]   - Parsed distance: ${totalDistance.value} km');
          print('[DISTANCE_CALC]   - Distance type: ${totalDistance.value.runtimeType}');
          
        } else {
          print('[DISTANCE_CALC] ❌ Missing location data, setting distance to 0');
          print('[DISTANCE_CALC]   - Customer location available: ${selectedAddress.value?.location?.latitude != null && selectedAddress.value?.location?.longitude != null}');
          print('[DISTANCE_CALC]   - Vendor location available: ${vendorModel.value.latitude != null && vendorModel.value.longitude != null}');
          print('[DISTANCE_CALC]   - Selected address: ${selectedAddress.value?.address}');
          print('[DISTANCE_CALC]   - Vendor model: ${vendorModel.value.title}');
          totalDistance.value = 0.0;
        }
        
        print('[DISTANCE_CALC] ==========================================');
        print('[DISTANCE_CALC] 🎯 FINAL DISTANCE RESULT: ${totalDistance.value} km');
        print('[DISTANCE_CALC] ==========================================');
        /*
  ...
  */
    ...
    */
    ...
    // Check if cart has promotional items or mart items
    final hasPromotionalItemsForTax = cartItem.any((item) =>
        item.promoId != null && item.promoId!.isNotEmpty);
    final hasMartItems = hasMartItemsInCart();
      ...
      for (var element in Constant.taxList!) {
        if ((element.title?.toLowerCase() ?? '').contains('sgst')) {
          ...
          if (hasPromotionalItemsForTax) {
            print('[PROMOTIONAL_TAX] SGST (5%) on item total: ' + sgst.toString());
          } else if (hasMartItems) {
          ...
          if (hasPromotionalItemsForTax) {
            print('[PROMOTIONAL_TAX] GST (18%) on delivery fee: ' + gst.toString());
          } else if (hasMartItems) {
    ...
    if (hasPromotionalItemsForTax) {
      print('[PROMOTIONAL_TAX] Total Taxes & Charges = ' + taxAmount.value.toString());
    } else if (hasMartItems) {
      print('[MART_TAX] Total Taxes & Charges = ' + taxAmount.value.toString());
    } else {
    ...
    if (cartItem.isNotEmpty && selectedFoodType.value == "Delivery") {
      ...
      if (hasPromotionalItems) {
        ...
        if (subTotal.value >= itemThreshold && totalDistance.value <= freeDeliveryKm) {
          isFreeDelivery = true;
          print('[MART_DELIVERY] Mart items - FREE DELIVERY eligible (₹${subTotal.value} >= ₹$itemThreshold, ${totalDistance.value} km <= ${freeDeliveryKm} km)');
        } else {
          isFreeDelivery = false;
          print('[MART_DELIVERY] Mart items - NO FREE DELIVERY (₹${subTotal.value} < ₹$itemThreshold or ${totalDistance.value} km > ${freeDeliveryKm} km)');
        }
      } else {
  ...
  void _calculateDeliveryCharge({
    ...
    print('$logPrefix Delivery settings - Free km: $freeDeliveryKm, Per km charge: $perKmCharge, Distance: ${totalDistance.value} km');
    ...
    if (vendorModel.value.isSelfDelivery == true && Constant.isSelfDeliveryFeature == true) {
      ...
      print('$logPrefix Free delivery within distance - showing original fee: ₹$baseCharge');
    } else {
      // Calculate extra charge for distance beyond free delivery
      double extraKm = (totalDistance.value - freeDeliveryKm).ceilToDouble();
      ...
      print('$logPrefix Extra delivery charge: $extraKm km × ₹$perKmCharge = ₹${deliveryCharges.value}');
      print('$logPrefix Original delivery fee for tax: ₹${originalDeliveryFee.value} (base ₹$baseCharge + extra ₹${deliveryCharges.value})');
    }
  }
  ...
  void calculateMartDeliveryCharge() {
    ...
    print('[MART_DELIVERY]   - Regular items: ${cartItem.length - martItems.length}');

    if (martItems.isEmpty) {
      print('[MART_DELIVERY] ❌ No mart items found, using regular delivery charge');
      calculateRegularDeliveryCharge();
      return;
    }
    ...
    print('[MART_DELIVERY]   - Is Self Delivery: ${vendorModel.value.isSelfDelivery}');
    print('[MART_DELIVERY]   - Self Delivery Feature: ${Constant.isSelfDeliveryFeature}');
  ...
  void _calculateMartDeliveryWithStaticValues() {
    ...
    print('[MART_DELIVERY]   - Is self delivery: ${vendorModel.value.isSelfDelivery}');
    print('[MART_DELIVERY]   - Self delivery feature enabled: ${Constant.isSelfDeliveryFeature}');

    print('[MART_DELIVERY] 🔍 DELIVERY LOGIC ANALYSIS:');
    print('[MART_DELIVERY]   - Subtotal (₹$subtotal) >= Threshold (₹$threshold): ${subtotal >= threshold}');
    print('[MART_DELIVERY]   - Distance (${distance} km) <= Free Distance (${freeKm} km): ${distance <= freeKm}');
    print('[MART_DELIVERY]   - Self Delivery: ${vendorModel.value.isSelfDelivery == true && Constant.isSelfDeliveryFeature == true}');

    if (vendorModel.value.isSelfDelivery == true && Constant.isSelfDeliveryFeature == true) {
      deliveryCharges.value = 0.0;
      originalDeliveryFee.value = 0.0;
      print('[MART_DELIVERY] ✅ RESULT: Self delivery - NO CHARGE');
      print('[MART_DELIVERY]   - Final delivery charge: ₹${deliveryCharges.value}');
      print('[MART_DELIVERY]   - Original delivery fee: ₹${originalDeliveryFee.value}');
    } else if (subtotal >= threshold) {
      print('[MART_DELIVERY] 🎯 CASE: Above threshold (₹$subtotal >= ₹$threshold)');
      // Above threshold - free delivery within distance
      if (distance <= freeKm) {
        deliveryCharges.value = 0.0;
        originalDeliveryFee.value = baseCharge;
        print('[MART_DELIVERY] ✅ RESULT: FREE DELIVERY - Above threshold and within free distance');
        print('[MART_DELIVERY]   - Distance: ${distance} km <= ${freeKm} km (free distance)');
        print('[MART_DELIVERY]   - Final delivery charge: ₹${deliveryCharges.value}');
        print('[MART_DELIVERY]   - Original delivery fee: ₹${originalDeliveryFee.value}');
      } else {
        ...
        originalDeliveryFee.value = baseCharge + deliveryCharges.value;
        print('[MART_DELIVERY] ✅ RESULT: FREE DELIVERY WITH EXTRA CHARGE - Above threshold but beyond free distance');
        print('[MART_DELIVERY]   - Distance: ${distance} km > ${freeKm} km (free distance)');
        print('[MART_DELIVERY]   - Extra km: ${extraKm} km');
        print('[MART_DELIVERY]   - Extra charge: ${extraKm} km × ₹$perKm = ₹${deliveryCharges.value}');
        print('[MART_DELIVERY]   - Final delivery charge: ₹${deliveryCharges.value}');
        print('[MART_DELIVERY]   - Original delivery fee: ₹${originalDeliveryFee.value}');
      }
    } else {
      print('[MART_DELIVERY] 🎯 CASE: Below threshold (₹$subtotal < ₹$threshold)');
      // Below threshold - always charge delivery
      if (distance <= freeKm) {
        deliveryCharges.value = baseCharge;
        originalDeliveryFee.value = baseCharge;
        print('[MART_DELIVERY] ✅ RESULT: BASE CHARGE - Below threshold, within free distance');
        print('[MART_DELIVERY]   - Distance: ${distance} km <= ${freeKm} km (free distance)');
        print('[MART_DELIVERY]   - Final delivery charge: ₹${deliveryCharges.value}');
        print('[MART_DELIVERY]   - Original delivery fee: ₹${originalDeliveryFee.value}');
      } else {
        ...
        originalDeliveryFee.value = deliveryCharges.value;
        print('[MART_DELIVERY] ✅ RESULT: FULL CHARGE - Below threshold, beyond free distance');
        print('[MART_DELIVERY]   - Distance: ${distance} km > ${freeKm} km (free distance)');
        print('[MART_DELIVERY]   - Extra km: ${extraKm} km');
        print('[MART_DELIVERY]   - Base charge: ₹$baseCharge');
        print('[MART_DELIVERY]   - Extra charge: ${extraKm} km × ₹$perKm = ₹${extraKm * perKm}');
        print('[MART_DELIVERY]   - Final delivery charge: ₹${deliveryCharges.value}');
        print('[MART_DELIVERY]   - Original delivery fee: ₹${originalDeliveryFee.value}');
      }
    }
    ...
    print('[MART_DELIVERY]   - Final delivery charge: ₹${deliveryCharges.value}');
    print('[MART_DELIVERY]   - Original delivery fee: ₹${originalDeliveryFee.value}');
    print('[MART_DELIVERY]   - Is delivery free: ${deliveryCharges.value == 0.0 ? 'YES' : 'NO'}');
    print('[MART_DELIVERY] ==========================================');
  }
  ...
  Future<Map<String, dynamic>?> _fetchMartDeliveryChargeSettings() async {
    try {
      print('[MART_DELIVERY] 🔍 Fetching mart delivery settings from Firestore...');
      final doc = await FirebaseFirestore.instance
          .collection('settings')
      ...
      if (doc.exists) {
        ...
        print('[MART_DELIVERY]   - Base delivery charge: ₹${data['base_delivery_charge']}');
        print('[MART_DELIVERY]   - Free delivery distance: ${data['free_delivery_distance_km']} km');
        print('[MART_DELIVERY]   - Per km charge above free: ₹${data['per_km_charge_above_free_distance']}');
        print('[MART_DELIVERY]   - Item total threshold: ₹${data['item_total_threshold']}');
        print('[MART_DELIVERY]   - Min delivery charges: ${data['minimum_delivery_charges']}');
        print('[MART_DELIVERY]   - Min delivery charges within km: ${data['minimum_delivery_charges_within_km']}');
        print('[MART_DELIVERY]   - Is active: ${data['is_active']}');
        print('[MART_DELIVERY]   - Delivery promotion text: ${data['delivery_promotion_text']}');
        print('[MART_DELIVERY]   - Min order message: ${data['min_order_message']}');
        return data;
      } else {
      ...
      print('[MART_DELIVERY] ❌ Error fetching mart delivery charge settings: $e');
      return null;
    }
  }
  ...
  void _calculateMartDeliveryWithSettings(Map<String, dynamic> settings) {
    ...
    final baseDeliveryCharge = (settings['base_delivery_charge'] as num?)?.toDouble() ?? 23.0;
    final freeDeliveryDistanceKm = (settings['free_delivery_distance_km'] as num?)?.toDouble() ?? 7.0;
    final perKmChargeAboveFreeDistance = (settings['per_km_charge_above_free_distance'] as num?)?.toDouble() ?? 8.0;
    final itemTotalThreshold = (settings['item_total_threshold'] as num?)?.toDouble() ?? 99.0;
    final minimumDeliveryCharges = (settings['minimum_delivery_charges'] as num?)?.toDouble();
    final minimumDeliveryChargesWithinKm = (settings['minimum_delivery_charges_within_km'] as num?)?.toDouble();
    ...
    print('[MART_DELIVERY]   - Free delivery distance: ${freeDeliveryDistanceKm} km');
    print('[MART_DELIVERY]   - Per km charge above free: ₹$perKmChargeAboveFreeDistance');
    print('[MART_DELIVERY]   - Item total threshold: ₹$itemTotalThreshold');
    print('[MART_DELIVERY]   - Min delivery charges: ${minimumDeliveryCharges ?? 'Not set'}');
    print('[MART_DELIVERY]   - Min delivery charges within km: ${minimumDeliveryChargesWithinKm ?? 'Not set'}');
    ...
    print('[MART_DELIVERY] 🔍 DELIVERY LOGIC ANALYSIS:');
    print('[MART_DELIVERY]   - Subtotal (₹${subTotal.value}) >= Threshold (₹$itemTotalThreshold): ${subTotal.value >= itemTotalThreshold}');
    print('[MART_DELIVERY]   - Distance (${totalDistance.value} km) <= Free Distance (${freeDeliveryDistanceKm} km): ${totalDistance.value <= freeDeliveryDistanceKm}');
    print('[MART_DELIVERY]   - Self Delivery: ${vendorModel.value.isSelfDelivery == true && Constant.isSelfDeliveryFeature == true}');
    ...
    if (vendorModel.value.isSelfDelivery == true && Constant.isSelfDeliveryFeature == true) {
      ...
      print('[MART_DELIVERY] 🎯 CASE: Above threshold (₹${subTotal.value} >= ₹$itemTotalThreshold)');
      // Above threshold - free delivery within distance
      if (totalDistance.value <= freeDeliveryDistanceKm) {
        deliveryCharges.value = 0.0;
        originalDeliveryFee.value = baseDeliveryCharge;
        print('[MART_DELIVERY] ✅ RESULT: FREE DELIVERY - Above threshold and within free distance');
        print('[MART_DELIVERY]   - Distance: ${totalDistance.value} km <= ${freeDeliveryDistanceKm} km (free distance)');
        print('[MART_DELIVERY]   - Final delivery charge: ₹${deliveryCharges.value}');
        print('[MART_DELIVERY]   - Original delivery fee: ₹${originalDeliveryFee.value}');
      } else {
        double extraKm = (totalDistance.value - freeDeliveryDistanceKm).ceilToDouble();
        ...
        print('[MART_DELIVERY] ✅ RESULT: PARTIAL CHARGE - Above threshold but beyond free distance');
        print('[MART_DELIVERY]   - Distance: ${totalDistance.value} km > ${freeDeliveryDistanceKm} km (free distance)');
        print('[MART_DELIVERY]   - Extra km: ${extraKm} km');
        print('[MART_DELIVERY]   - Extra charge: ${extraKm} km × ₹$perKmChargeAboveFreeDistance = ₹${deliveryCharges.value}');
        print('[MART_DELIVERY]   - Final delivery charge: ₹${deliveryCharges.value}');
        print('[MART_DELIVERY]   - Original delivery fee: ₹${originalDeliveryFee.value}');
      }
    } else {
      print('[MART_DELIVERY] 🎯 CASE: Below threshold (₹${subTotal.value} < ₹$itemTotalThreshold)');
      // Below threshold - always charge delivery
      if (totalDistance.value <= freeDeliveryDistanceKm) {
        ...
        double extraKm = (totalDistance.value - freeDeliveryDistanceKm).ceilToDouble();
        deliveryCharges.value = baseDeliveryCharge + (extraKm * perKmChargeAboveFreeDistance);
        originalDeliveryFee.value = deliveryCharges.value;
        print('[MART_DELIVERY] ✅ RESULT: FULL CHARGE - Below threshold, beyond free distance');
        print('[MART_DELIVERY]   - Distance: ${totalDistance.value} km > ${freeDeliveryDistanceKm} km (free distance)');
        print('[MART_DELIVERY]   - Extra km: ${extraKm} km');
        print('[MART_DELIVERY]   - Base charge: ₹$baseDeliveryCharge');
        print('[MART_DELIVERY]   - Extra charge: ${extraKm} km × ₹$perKmChargeAboveFreeDistance = ₹${extraKm * perKmChargeAboveFreeDistance}');
        print('[MART_DELIVERY]   - Final delivery charge: ₹${deliveryCharges.value}');
        print('[MART_DELIVERY]   - Original delivery fee: ₹${originalDeliveryFee.value}');
      }
    }
    ...
    if (minimumDeliveryCharges != null && deliveryCharges.value < minimumDeliveryCharges) {
      print('[MART_DELIVERY]   - Current charge (₹${deliveryCharges.value}) < Min charge (₹$minimumDeliveryCharges)');
      deliveryCharges.value = minimumDeliveryCharges;
      print('[MART_DELIVERY] ✅ Applied minimum delivery charge: ₹$minimumDeliveryCharges');
    } else {
      print('[MART_DELIVERY]   - No minimum delivery charge applied (not set or not needed)');
    }

    if (minimumDeliveryChargesWithinKm != null && totalDistance.value <= freeDeliveryDistanceKm && deliveryCharges.value < minimumDeliveryChargesWithinKm) {
      print('[MART_DELIVERY]   - Within free distance and current charge (₹${deliveryCharges.value}) < Min charge within km (₹$minimumDeliveryChargesWithinKm)');
      deliveryCharges.value = minimumDeliveryChargesWithinKm;
      print('[MART_DELIVERY] ✅ Applied minimum delivery charge within free distance: ₹$minimumDeliveryChargesWithinKm');
    } else {
      print('[MART_DELIVERY]   - No minimum delivery charge within km applied (not set or not needed)');
    }
  ...
  void _calculateMartDeliveryWithDefaults() {
    ...
    print('[MART_DELIVERY] Base charge: ₹$baseCharge, Per km: ₹$perKmCharge, Free km: ${freeDeliveryKm}km, Threshold: ₹$itemThreshold, Distance: ${totalDistance.value} km');
    ...
    if (vendorModel.value.isSelfDelivery == true && Constant.isSelfDeliveryFeature == true) {
      ...
      if (totalDistance.value <= freeDeliveryKm) {
        deliveryCharges.value = 0.0;
        originalDeliveryFee.value = baseCharge;
        print('[MART_DELIVERY] Default: Free delivery - above threshold and within free distance');
      } else {
        double extraKm = (totalDistance.value - freeDeliveryKm).ceilToDouble();
        deliveryCharges.value = extraKm * perKmCharge;
        originalDeliveryFee.value = baseCharge + deliveryCharges.value;
        print('[MART_DELIVERY] Default: Above threshold but beyond free distance: ${extraKm} km × ₹$perKmCharge = ₹${deliveryCharges.value}');
      }
    } else {
      // Below threshold - always charge delivery
      if (totalDistance.value <= freeDeliveryKm) {
        deliveryCharges.value = baseCharge;
        originalDeliveryFee.value = baseCharge;
        print('[MART_DELIVERY] Default: Below threshold, within free distance: ₹$baseCharge');
      } else {
        double extraKm = (totalDistance.value - freeDeliveryKm).ceilToDouble();
        deliveryCharges.value = baseCharge + (extraKm * perKmCharge);
        originalDeliveryFee.value = deliveryCharges.value;
        print('[MART_DELIVERY] Default: Below threshold, beyond free distance: ₹$baseCharge + (${extraKm} km × ₹$perKmCharge) = ₹${deliveryCharges.value}');
      }
    }
  ...
  Future<bool> addToCart(
    ...
    if (isIncrement) {
      ...
      final success = await cartProvider.addToCart(Get.context!, cartProductModel, quantity);
      if (!success) {
  ...
  placeOrder() async {
    ...
    try {
      ...
      if (isPaymentCompleted.value && _lastPaymentId != null && 
          (selectedPaymentMethod.value.isEmpty || selectedPaymentMethod.value == PaymentGateway.cod.name)) {
        selectedPaymentMethod.value = PaymentGateway.razorpay.name;
        print('🔑 Payment method corrected in placeOrder: ${selectedPaymentMethod.value}');
      }
      ...
      if (e.toString().contains('Delivery zone validation failed') || 
        ...
        print('DEBUG: Zone validation failed - specific error message already shown');
      } else {
        // Generic order error
  ...
  Future<bool> validateOrderBeforePayment() async {
    try {
      ...
      print('DEBUG: ✅ Address validation passed - continuing with order validation');
        ...
        if (latestVendor != null) {
          ...
          if (latestVendor.vType == 'mart') {
            ...
            if (latestVendor.isOpen == false) {
              ShowToastDialog.showToast("Jippy Mart is temporarily closed. Please try again later.");
              return false;
            }
          } else {
  ...
  Future<void> _setOrderInternal() async {
    ...
    try {
      ...
      print('DEBUG: Order address set - Address: ${selectedAddress.value!.address}, Locality: ${selectedAddress.value!.locality}');
      ...
      if (vendorModel.value.id != null) {
        ...
        try {
          ...
          print('DEBUG: Error fetching mart vendor: $e, using default vendor object');
          orderModel.vendorID = 'mart_default';
          // Create a default vendor object instead of setting to null
          orderModel.vendor = VendorModel(
            id: 'mart_default',
      ...
      if (isPaymentCompleted.value && _lastPaymentId != null) {
        print('🔑 Payment was successful but order failed - showing retry options');
        // Don't reset payment state here - let user retry
        ShowToastDialog.showToast("Order placement failed. Your payment is safe. Please try again.".tr);
      } else {
  ...
  getPaymentSettings() async {
    await FireStoreUtils.getPaymentSettingsData().then(
        ...
        } else if (cashOnDeliverySettingModel.value.isEnabled == true && subTotal.value <= 599 && !hasMartItemsInCart()) {
  ...
  void openCheckout({required amount, required orderId}) async {
    ...
    print('DEBUG: Razorpay Key: ${razorPayModel.value.razorpayKey}');
    print('DEBUG: Payment state - InProgress: ${isPaymentInProgress.value}, Completed: ${isPaymentCompleted.value}');

    // 🔑 CHECK PAYMENT STATE BEFORE OPENING
    if (isPaymentInProgress.value) {
      print('🔑 WARNING: Payment already in progress, blocking duplicate payment');
      ShowToastDialog.showToast("Payment is already in progress. Please wait...".tr);
      return;
    }

    if (isPaymentCompleted.value) {
      print('🔑 WARNING: Payment already completed, blocking duplicate payment');
      ShowToastDialog.showToast("Payment already completed. Please refresh the page.".tr);
      return;
    }
    ...
    if (!_razorpayCrashPrevention.isInitialized) {
      ...
      if (!initialized) {
        ...
        ShowToastDialog.showToast("Payment system is temporarily unavailable. Please try again later.".tr);
        return;
      }
    }
    ...
    try {
      ...
      if (success) {
        ...
        isPaymentInProgress.value = false;
        ShowToastDialog.showToast("Failed to open payment gateway. Please try again.".tr);
      }
    } catch (e) {
  ...
  void handlePaymentSuccess(PaymentSuccessResponse response) {
    try {
      ...
      isPaymentInProgress.value = false;
      ShowToastDialog.showToast("Payment processing failed. Please try again.".tr);
    }
  }
  ...
  void handleExternalWallet(ExternalWalletResponse response) {
    try {
      ...
      ShowToastDialog.showToast("External wallet selected: ${response.walletName}".tr);
    } catch (e) {
  ...
  Future<void> _processOrderWithRetry() async {
    ...
    while (retryCount < maxRetries) {
      try {
        print('🔑 Attempting order placement - Retry ${retryCount + 1}/$maxRetries');
        ...
        ShowToastDialog.showLoader("Retrying order placement... (${retryCount}/$maxRetries)".tr);
      }
    }
  }
    ...
    Get.dialog(
      AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text("Your payment was successful, but we couldn't place your order."),
            SizedBox(height: 10),
            Text("Don't worry - your money is safe and will be refunded within 24 hours."),
            SizedBox(height: 10),
  ...
  void resetPaymentState() {
    ...
    ShowToastDialog.showToast("Payment state reset. You can try payment again.".tr);
  }
  ...
  void _savePaymentStateSync() {
    try {
      ...
      Preferences.setString(_paymentStateKey, isPaymentInProgress.value.toString());
      ...
      if (_lastPaymentTime != null) {
        Preferences.setString(_paymentTimeKey, _lastPaymentTime!.millisecondsSinceEpoch.toString());
      }
  ...
  Future<void> _savePaymentState() async {
    try {
      ...
      await Preferences.setString(_paymentStateKey, isPaymentInProgress.value.toString());
      if (_lastPaymentId != null) {
        await Preferences.setString(_paymentIdKey, _lastPaymentId!);
      }
      if (_lastPaymentSignature != null) {
        await Preferences.setString(_paymentSignatureKey, _lastPaymentSignature!);
      }
      if (_lastPaymentTime != null) {
        await Preferences.setString(_paymentTimeKey, _lastPaymentTime!.millisecondsSinceEpoch.toString());
      }
      // 🔑 SAVE PAYMENT METHOD TO PERSISTENT STORAGE
      if (selectedPaymentMethod.value.isNotEmpty) {
        await Preferences.setString(_paymentMethodKey, selectedPaymentMethod.value);
  ...
  Future<void> _restorePaymentState() async {
    try {
      ...
      if (paymentState == 'true') {
        ...
        if (paymentTimeStr != null && paymentTimeStr.isNotEmpty && paymentTimeStr != '') {
          _lastPaymentTime = DateTime.fromMillisecondsSinceEpoch(int.parse(paymentTimeStr));
          print('🔑 Restored Payment Time: $_lastPaymentTime');
        }
        
        // 🔑 RESTORE PAYMENT METHOD FROM PERSISTENT STORAGE
        if (paymentMethodStr != null && paymentMethodStr.isNotEmpty && paymentMethodStr != '') {
          ...
          selectedPaymentMethod.value = PaymentGateway.razorpay.name;
          print('🔑 Payment method inferred from payment ID: ${selectedPaymentMethod.value}');
        }
    ...
    try {
      ...
      if (_lastPaymentTime != null) {
        ...
        if (timeSincePayment > paymentTimeout) {
          ...
          await _clearPersistentPaymentState();
          _resetPaymentState();
          ShowToastDialog.showToast("Payment session expired. Please try again.".tr);
          return;
        }
      }
  ...
  Future<void> _completePendingOrder() async {
    try {
      ...
      ShowToastDialog.closeLoader();
      ShowToastDialog.showToast("Failed to complete order. Please try again.".tr);
      await _clearPersistentPaymentState();
      _resetPaymentState();
    }
  }
  ...
  placeOrderAfterPayment() async {
    ...
    try {
      ...
      // 🔑 ENSURE PAYMENT METHOD IS SET CORRECTLY FOR PREPAID ORDERS
      if (selectedPaymentMethod.value.isEmpty || selectedPaymentMethod.value == PaymentGateway.cod.name) {
  ...
  Future<bool> isMinimumOrderMet() async {
    try {
      // Check if cart contains any mart items
      bool hasMartItems = cartItem.any((item) => item.vendorID?.startsWith('mart_') == true);
  ...
  Future<void> validateMinimumOrderValue() async {
    try {
      ...
      if (hasMartItems) {
        final martItems = cartItem.where((item) => item.vendorID?.startsWith('mart_') == true).toList();
        print('[MIN_ORDER_VALIDATION]   - Mart items count: ${martItems.length}');
        for (int i = 0; i < martItems.length; i++) {
          final item = martItems[i];
          print('[MIN_ORDER_VALIDATION]   - Mart item ${i + 1}: ${item.name} (₹${item.price}) x${item.quantity}');
        }
      }
      
      if (!hasMartItems) {
        print('[MIN_ORDER_VALIDATION] ✅ No mart items in cart, skipping minimum order validation');
        return;
      }

      print('[MIN_ORDER_VALIDATION] 🔍 Cart contains mart items, validating minimum order value...');
      ...
      if (_martDeliverySettings != null) {
        ...
        minOrderValue = (_martDeliverySettings!['min_order_value'] as num?)?.toDouble() ?? 99.0;
        minOrderMessage = _martDeliverySettings!['min_order_message'] ?? 'Min Item value is ₹${minOrderValue.toInt()}';
        print('DEBUG: Using martDeliveryCharge settings for minimum order validation');
        print('DEBUG: Settings active: $isSettingsActive, Min order value: ₹$minOrderValue');
      } else {
        // Fetch settings if not already loaded
        print('DEBUG: Fetching martDeliveryCharge settings for minimum order validation...');
        final settings = await _fetchMartDeliveryChargeSettings();
        if (settings != null) {
          _martDeliverySettings = settings;
          isSettingsActive = settings['is_active'] ?? true;
          minOrderValue = (settings['min_order_value'] as num?)?.toDouble() ?? 99.0;
          minOrderMessage = settings['min_order_message'] ?? 'Min Item value is ₹${minOrderValue.toInt()}';
          print('DEBUG: Fetched settings - Active: $isSettingsActive, Min order value: ₹$minOrderValue');
        }
      }
      
      // Check if settings are active
      if (!isSettingsActive) {
        print('[MIN_ORDER_VALIDATION] ⚠️ Mart delivery settings are inactive, skipping minimum order validation');
      ...
      print('[MIN_ORDER_VALIDATION]   - Difference needed: ₹${(minOrderValue - currentSubTotal).toStringAsFixed(2)}');
      ...
      if (currentSubTotal < minOrderValue) {
        ...
        print('[MIN_ORDER_VALIDATION]   - Current subtotal (₹$currentSubTotal) < Minimum required (₹$minOrderValue)');
        print('[MIN_ORDER_VALIDATION]   - Short by: ₹${(minOrderValue - currentSubTotal).toStringAsFixed(2)}');
        print('[MIN_ORDER_VALIDATION]   - Showing error message: $minOrderMessage');
  ...
  bool _isRepeatedFailedAttempt() {
    ...
    if (_lastFailedAddressId == currentAddressId) {
      ...
      if (_lastFailedValidationTime != null && 
        ...
        _failedAttempts++;
        print('DEBUG: Repeated failed attempt #$_failedAttempts for address: $currentAddressId');
        return true;
      }
    }
  ...
  void _recordFailedValidation() {
    if (selectedAddress.value?.id != null) {
      ...
      _failedAttempts++;
      print('DEBUG: Recorded failed validation attempt #$_failedAttempts for address: $_lastFailedAddressId');
    }
  }
  ...
  Future<bool> _validateAddressBulletproof() async {
    ...
    try {
      print('🏠 [BULLETPROOF_ADDRESS] ==========================================');
      print('🏠 [BULLETPROOF_ADDRESS] VALIDATION STARTED at ${startTime.toIso8601String()}');
      print('🏠 [BULLETPROOF_ADDRESS] Address count in list: ${Constant.userModel?.shippingAddress?.length ?? 0}');
      
      // CRITICAL CHECK 1: Address must exist
      if (selectedAddress.value == null) {
        print('🏠 [BULLETPROOF_ADDRESS] ❌ CHECK 1 FAILED - No address selected');
        print('🏠 [BULLETPROOF_ADDRESS] Selected address: NULL');
        print('🏠 [BULLETPROOF_ADDRESS] Available addresses: ${Constant.userModel?.shippingAddress?.length ?? 0}');
        ShowToastDialog.showToast("Delivery address is required. Please add an address to continue.".tr);
        Get.to(() => const AddressListScreen());
        return false;
      }
      ...
      print('🏠 [BULLETPROOF_ADDRESS] Coordinates: lat=${address.location?.latitude}, lng=${address.location?.longitude}');
      ...
      if (address.id == null || address.id!.trim().isEmpty) {
        ...
        print('🏠 [BULLETPROOF_ADDRESS] Address ID: "${address.id}" (null or empty)');
        ShowToastDialog.showToast("Invalid address detected. Please select a valid delivery address.".tr);
        Get.to(() => const AddressListScreen());
        return false;
      }
      print('🏠 [BULLETPROOF_ADDRESS] ✅ CHECK 2 PASSED - Valid address ID: "${address.id}"');
      ...
      if (address.address == null || 
        ...
        print('🏠 [BULLETPROOF_ADDRESS] ❌ CHECK 3 FAILED - Invalid address field');
        print('🏠 [BULLETPROOF_ADDRESS] Address field: "${address.address}" (null or empty)');
      ...
      if (address.address!.trim() == 'Current Location' && 
          (address.location?.latitude == null || address.location?.longitude == null)) {
        print('🏠 [BULLETPROOF_ADDRESS] ❌ CHECK 3 FAILED - Current Location without coordinates');
        print('🏠 [BULLETPROOF_ADDRESS] Address: "${address.address}" but no valid coordinates');
        ShowToastDialog.showToast("Current location address must have valid coordinates. Please add a proper address.".tr);
        Get.to(() => const AddressListScreen());
        return false;
      }
      
      print('🏠 [BULLETPROOF_ADDRESS] ✅ CHECK 3 PASSED - Valid address field: "${address.address}"');
      ...
      if (address.locality == null || 
        ...
        print('🏠 [BULLETPROOF_ADDRESS] Locality: "${address.locality}" (null or empty)');
        ShowToastDialog.showToast("Please select a valid delivery address with complete location details.".tr);
        Get.to(() => const AddressListScreen());
        return false;
      }
      ...
      if (address.locality!.trim() == 'Current Location' && 
          (address.location?.latitude == null || address.location?.longitude == null)) {
        print('🏠 [BULLETPROOF_ADDRESS] ❌ CHECK 4 FAILED - Current Location locality without coordinates');
        print('🏠 [BULLETPROOF_ADDRESS] Locality: "${address.locality}" but no valid coordinates');
        ShowToastDialog.showToast("Current location must have valid coordinates. Please add a proper address.".tr);
        Get.to(() => const AddressListScreen());
        return false;
      }
      
      print('🏠 [BULLETPROOF_ADDRESS] ✅ CHECK 4 PASSED - Valid locality: "${address.locality}"');
      ...
      if (address.location == null ||
        ...
        print('🏠 [BULLETPROOF_ADDRESS] ❌ CHECK 5 FAILED - Invalid coordinates');
        print('🏠 [BULLETPROOF_ADDRESS] Location: ${address.location}');
        print('🏠 [BULLETPROOF_ADDRESS] Latitude: ${address.location?.latitude}');
        print('🏠 [BULLETPROOF_ADDRESS] Longitude: ${address.location?.longitude}');
        ShowToastDialog.showToast("Please select a delivery address with valid location coordinates.".tr);
        Get.to(() => const AddressListScreen());
        return false;
      }
      print('🏠 [BULLETPROOF_ADDRESS] ✅ CHECK 5 PASSED - Valid coordinates: lat=${address.location!.latitude}, lng=${address.location!.longitude}');
      ...
      if (address.id!.startsWith('fallback_zone_') || 
        ...
        print('🏠 [BULLETPROOF_ADDRESS] ❌ CHECK 6 FAILED - FALLBACK ZONE DETECTED');
        print('🏠 [BULLETPROOF_ADDRESS] Address ID: "${address.id}"');
        print('🏠 [BULLETPROOF_ADDRESS] Address: "${address.address}"');
        print('🏠 [BULLETPROOF_ADDRESS] Locality: "${address.locality}"');
        print('🏠 [BULLETPROOF_ADDRESS] ERROR: Fallback zones are not allowed for orders!');
        ShowToastDialog.showToast("Please add a valid delivery address. Fallback zones are not allowed.".tr);
        Get.to(() => const AddressListScreen());
        return false;
      }
      ...
      print('🏠 [BULLETPROOF_ADDRESS] Checking coordinate bounds - lat: $lat, lng: $lng');
      print('🏠 [BULLETPROOF_ADDRESS] India bounds: lat (6.0-37.0), lng (68.0-97.0)');
      
      if (lat < 6.0 || lat > 37.0 || lng < 68.0 || lng > 97.0) {
        print('🏠 [BULLETPROOF_ADDRESS] ❌ CHECK 7 FAILED - Coordinates outside India bounds');
        print('🏠 [BULLETPROOF_ADDRESS] Latitude: $lat (valid: 6.0-37.0) - ${lat >= 6.0 && lat <= 37.0 ? "✅" : "❌"}');
        print('🏠 [BULLETPROOF_ADDRESS] Longitude: $lng (valid: 68.0-97.0) - ${lng >= 68.0 && lng <= 97.0 ? "✅" : "❌"}');
        ShowToastDialog.showToast("Please select a delivery address within our service area.".tr);
        Get.to(() => const AddressListScreen());
        return false;
      }
      print('🏠 [BULLETPROOF_ADDRESS] ✅ CHECK 7 PASSED - Coordinates within India bounds');
      
      // CRITICAL CHECK 8: ZONE VALIDATION - Address zone must match vendor zone
      print('🏠 [BULLETPROOF_ADDRESS] Starting zone validation...');
      print('🏠 [BULLETPROOF_ADDRESS] Address zone: ${address.zoneId ?? "NULL"}');
      print('🏠 [BULLETPROOF_ADDRESS] Vendor zone: ${vendorModel.value.zoneId ?? "NULL"}');
      
      if (address.zoneId == null || address.zoneId!.isEmpty) {
        print('🏠 [BULLETPROOF_ADDRESS] ⚠️ Address zone ID is null - attempting to detect...');
        ...
        if (detectedZoneId != null) {
          ...
          print('🏠 [BULLETPROOF_ADDRESS] ❌ CHECK 8 FAILED - Could not detect zone ID');
          ShowToastDialog.showToast("Address zone not detected. Please update your address or contact support.".tr);
          Get.to(() => const AddressListScreen());
          return false;
        }
      }
      
      if (vendorModel.value.zoneId == null || vendorModel.value.zoneId!.isEmpty) {
        print('🏠 [BULLETPROOF_ADDRESS] ❌ CHECK 8 FAILED - Vendor zone ID is null');
        print('🏠 [BULLETPROOF_ADDRESS] Vendor zone ID: "${vendorModel.value.zoneId}"');
        ShowToastDialog.showToast("Vendor zone not configured. Please contact support.".tr);
        return false;
      }
      ...
      if (address.zoneId != vendorModel.value.zoneId) {
        ...
        print('🏠 [BULLETPROOF_ADDRESS] Vendor zone: "${vendorModel.value.zoneId}"');
        print('🏠 [BULLETPROOF_ADDRESS] ERROR: Delivery not available to this address!');
      ...
      print('🏠 [BULLETPROOF_ADDRESS] ✅ CHECK 8 PASSED - Zone validation successful');
      print('🏠 [BULLETPROOF_ADDRESS] Address zone matches vendor zone: "${address.zoneId}"');
      ...
      if (vendorModel.value.latitude != null && vendorModel.value.longitude != null) {
        ...
        print('🏠 [BULLETPROOF_ADDRESS] Calculated distance: ${distance.toStringAsFixed(2)} km');
        print('🏠 [BULLETPROOF_ADDRESS] Vendor location: lat=${vendorModel.value.latitude}, lng=${vendorModel.value.longitude}');
        print('🏠 [BULLETPROOF_ADDRESS] Address location: lat=${address.location!.latitude}, lng=${address.location!.longitude}');
        ...
        if (distance > maxDeliveryDistance) {
          ...
          print('🏠 [BULLETPROOF_ADDRESS] Distance: ${distance.toStringAsFixed(2)} km');
          print('🏠 [BULLETPROOF_ADDRESS] Max allowed: $maxDeliveryDistance km');
          print('🏠 [BULLETPROOF_ADDRESS] ERROR: Address is too far from vendor location!');
        ...
        print('🏠 [BULLETPROOF_ADDRESS] ✅ CHECK 9 PASSED - Distance validation successful');
        print('🏠 [BULLETPROOF_ADDRESS] Distance: ${distance.toStringAsFixed(2)} km (within $maxDeliveryDistance km limit)');
      } else {
        print('🏠 [BULLETPROOF_ADDRESS] ⚠️ CHECK 9 SKIPPED - Vendor location not available');
      }
      ...
      print('🏠 [BULLETPROOF_ADDRESS] Total validation duration: ${totalDuration.inMilliseconds}ms');
      print('🏠 [BULLETPROOF_ADDRESS] ==========================================');
      ...
      print('🏠 [BULLETPROOF_ADDRESS] Total duration: ${totalDuration.inMilliseconds}ms');
      print('🏠 [BULLETPROOF_ADDRESS] Final result: ADDRESS_INVALID (ERROR)');
      print('🏠 [BULLETPROOF_ADDRESS] ==========================================');
      ShowToastDialog.showToast("Error validating address. Please select a valid delivery address.".tr);
      Get.to(() => const AddressListScreen());
      return false;
    }
  }
  ...
  Future<void> _validateDeliveryZone() async {
    try {
      ...
      if (selectedAddress.value?.zoneId != null && 
        ...
        if (vendorModel.value.zoneId != null && 
          ...
          if (selectedAddress.value!.zoneId != vendorModel.value.zoneId) {
            print('DEBUG: Zone mismatch - Address zone: ${selectedAddress.value!.zoneId}, Vendor zone: ${vendorModel.value.zoneId}');
            _recordFailedValidation();
            DeliveryZoneAlertDialog.showZoneMismatchError();
            throw Exception('Delivery zone validation failed');
          }
          
          print('DEBUG: Zone validation passed - Address and vendor in same zone: ${selectedAddress.value!.zoneId}');
          return;
        }
      }
      ...
      if (hasMartItemsInCart() && vendorModel.value.latitude == null && vendorModel.value.longitude == null) {
        print('DEBUG: Mart items with no vendor location - skipping zone validation');
        return;
      }
      
      // If no zone or distance validation possible, show warning but allow order
      print('DEBUG: No zone or distance validation possible - allowing order with warning');
