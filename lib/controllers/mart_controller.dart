...
class MartController extends GetxController {
  ...
  RxMap<String, List<MartSubcategoryModel>> subcategoriesMap = <String, List<MartSubcategoryModel>>{}.obs;
  ...
  RxMap<String, List<MartItemModel>> sectionProducts = <String, List<MartItemModel>>{}.obs;
  RxBool isSectionsLoading = false.obs;
  bool _sectionsLoadingTriggered = false; // Flag to prevent multiple loading attempts
  RxList<MartSubcategoryModel> subcategories = <MartSubcategoryModel>[].obs;
  
  // Delivery settings
  Rx<MartDeliverySettingsModel?> deliverySettings = Rx<MartDeliverySettingsModel?>(null);
  ...
  Rx<PageController> martTopBannerController = PageController(viewportFraction: 1.0).obs;
  Rx<PageController> martBottomBannerController = PageController(viewportFraction: 1.0).obs;
  ...
  List<MartItemModel> get filteredTrendingItems {
    final filtered = trendingItems.where((item) => item.isTrending == true).toList();
    print('[MART CONTROLLER] 🔍 Filtered trending items: ${filtered.length}/${trendingItems.length} (isTrending=true)');
    if (trendingItems.isNotEmpty) {
      print('[MART CONTROLLER] 📊 Trending items breakdown:');
      for (int i = 0; i < trendingItems.length; i++) {
        final item = trendingItems[i];
        print('[MART CONTROLLER]   ${i + 1}. ${item.name} - isTrending: ${item.isTrending}');
      }
    }
    return filtered;
  }
  ...
  void onInit() {
    ...
    print('[MART CONTROLLER]   - featuredCategories: ${featuredCategories.length}');
    print('[MART CONTROLLER]   - isCategoryLoading: ${isCategoryLoading.value}');
  ...
  void startMartBannerTimer() {
    ...
    _martBannerTimer = Timer.periodic(const Duration(seconds: 3), (timer) {
      ...
      if (!Get.isRegistered<MartController>() || !martTopBannerController.value.hasClients) {
        timer.cancel();
        return;
      }
  ...
  void _initializeBannerStreams() {
    ...
    FireStoreUtils.getMartBottomBannersStream().listen(
      (banners) {
        print('[MART CONTROLLER] Lazy load - Bottom banners: ${banners.length}');
  ...
  void _initializeBannerControllers() {
    ...
    if (martTopBanners.isNotEmpty && martTopBanners.length > 1) {
      int middlePosition = (martTopBanners.length * 1000) ~/ 2; // Start at middle of infinite list
      martTopBannerController.value = PageController(initialPage: middlePosition);
    ...
    if (martBottomBanners.isNotEmpty && martBottomBanners.length > 1) {
      int middlePosition = (martBottomBanners.length * 1000) ~/ 2; // Start at middle of infinite list
      martBottomBannerController.value = PageController(initialPage: middlePosition);
    ...
    print('[MART CONTROLLER] Banner controllers initialized for infinite scrolling');
  }
  ...
  Future<void> _initializeServices() async {
    try {
      print('[MART CONTROLLER] ==========================================');
      print('[MART CONTROLLER] _initializeServices() called - FIREBASE ONLY MODE');
      print('[MART CONTROLLER] ==========================================');
      ...
      print('[MART CONTROLLER] ✅ Streaming initialization started - data will load progressively');
    } catch (e) {
  ...
  Future<void> _loadVendorsInBackground() async {
    try {
      print('[MART CONTROLLER] 🔄 Loading vendors in background from Firestore...');
      ...
      if (vendors.isNotEmpty) {
        print('[MART CONTROLLER] ✅ Loaded ${vendors.length} vendors from Firestore');
        martVendors.clear();
        ...
        if (selectedVendorId.value.isEmpty) {
          ...
          print('[MART CONTROLLER] 🎯 Auto-selected first vendor: ${vendors.first.name}');
  ...
  Future<void> loadMartVendors({bool refresh = false}) async {
    try {
      ...
      try {
        ...
        print('[MART CONTROLLER] 📍 Found user location: ${location.latitude}, ${location.longitude}');
        
        if (timeoutReached) return;
        
        print('[MART CONTROLLER] 📞 Calling _martService.getMartVendors() with:');
        ...
        print('[MART CONTROLLER] ✅ Vendors loaded successfully: ${vendors.length} vendors');
        ...
        if (selectedVendorId.value.isEmpty && vendors.isNotEmpty) {
          ...
          print('[MART CONTROLLER] 🎯 Auto-selected first vendor: ${firstVendor.name} (${firstVendor.id})');
        }
        ...
        errorMessage.value = "Unable to load stores. Please check your connection and try again.";
        
        // If no vendors loaded, try to continue with empty state
        if (martVendors.isEmpty) {
          print('[MART CONTROLLER] ⚠️ No vendors available, continuing with empty state');
        }
      } finally {
        timeoutTimer.cancel();
        if (!timeoutReached) {
  ...
  Future<void> loadMartCategories({bool refresh = false}) async {
    ...
    try {
      ...
      if (refresh) {
        print('[MART CONTROLLER] 🔄 Refresh mode - clearing existing categories');
      ...
      final categories = await _firestoreService.getHomepageCategories(limit: 50);
      print('[MART CONTROLLER] ✅ Categories loaded from Firestore: ${categories.length} categories');
      
      if (categories.isNotEmpty) {
        // ✅ Load categories FIRST - show them immediately
        martCategories.assignAll(categories);
        print('[MART CONTROLLER] ✅ Categories loaded successfully: ${categories.length} categories');
  ...
  Future<void> _loadItemCountsInBackground(List<MartCategoryModel> categoriesToLoad) async {
    try {
      print('[MART CONTROLLER] 🔄 Loading item counts in background from Firestore...');
      
      for (int i = 0; i < categoriesToLoad.length; i++) {
        final category = categoriesToLoad[i];
        try {
          // Get item count from Firestore
          final itemCount = await _firestoreService.getItemCountForCategory(category.id!);
          category.itemCount = itemCount;
          print('[MART CONTROLLER] ✅ Category "${category.title}" has $itemCount items');
          
          // Update the UI for this specific category
          martCategories.firstWhereOrNull((c) => c.id == category.id)?.itemCount = itemCount;
          
        } catch (e) {
          print('[MART CONTROLLER] ⚠️ Failed to get item count for category "${category.title}": $e');
          martCategories.firstWhereOrNull((c) => c.id == category.id)?.itemCount = 0;
        }
      }
  ...
  Future<void> loadSubcategoriesForCategories() async {
    try {
      print('[MART CONTROLLER] 🔄 Loading sub-categories for all categories...');
      ...
      for (final category in featuredCategories) {
        if (category.id != null) {
          try {
            print('[MART CONTROLLER] 🔄 Loading subcategories for category: ${category.title} (ID: ${category.id})');
            await loadSubcategoriesForCategory(category.id!);
          } catch (e) {
            print('[MART CONTROLLER] ⚠️ Error loading subcategories for ${category.title}: $e');
          }
        }
      }
      
      print('[MART CONTROLLER] ✅ Sub-categories loading completed. Total categories processed: ${featuredCategories.length}');
      print('[MART CONTROLLER] 📊 Subcategories map contains: ${subcategoriesMap.length} category entries');
      subcategoriesMap.forEach((categoryId, subcategoryList) {
        print('[MART CONTROLLER] 📊 Category $categoryId: ${subcategoryList.length} subcategories');
      });
    } catch (e) {
  ...
  Future<void> loadAllHomepageSubcategories() async {
    try {
      print('[MART CONTROLLER] 🔄 Loading all homepage subcategories directly from Firestore...');
      isSubcategoryLoading.value = true;
      ...
      final allSubcategories = await _firestoreService.getAllHomepageSubcategories();
      ...
      if (allSubcategories.isNotEmpty) {
        ...
        print('[MART CONTROLLER] ✅ Loaded ${allSubcategories.length} homepage subcategories from ${groupedSubcategories.length} parent categories');
        print('[MART CONTROLLER] 📊 Subcategories map contains: ${subcategoriesMap.length} category entries');
        subcategoriesMap.forEach((categoryId, subcategoryList) {
          print('[MART CONTROLLER] 📊 Category $categoryId: ${subcategoryList.length} subcategories');
        });
  ...
  Future<void> loadAllSubcategoriesDebug() async {
    try {
      print('[MART CONTROLLER] 🔍 DEBUG: Loading ALL subcategories from Firestore (no filters)...');
      isSubcategoryLoading.value = true;
      ...
      final allSubcategories = await _firestoreService.getAllSubcategoriesDebug();
      ...
      if (allSubcategories.isNotEmpty) {
        ...
        print('[MART CONTROLLER] 🔍 DEBUG: Loaded ${allSubcategories.length} total subcategories from ${groupedSubcategories.length} parent categories');
        print('[MART CONTROLLER] 🔍 DEBUG: Subcategories map contains: ${subcategoriesMap.length} category entries');
        subcategoriesMap.forEach((categoryId, subcategoryList) {
          print('[MART CONTROLLER] 🔍 DEBUG: Category $categoryId: ${subcategoryList.length} subcategories');
        });
  ...
  Future<void> loadSubcategoriesForCategory(String categoryId) async {
    try {
      print('[MART CONTROLLER] 🔄 Loading sub-categories for category: $categoryId');
      ...
      print('[MART CONTROLLER] ✅ Loaded ${subcategories.length} sub-categories for category: $categoryId');
    } catch (e) {
      print('[MART CONTROLLER] ❌ Error loading sub-categories for category $categoryId: $e');
  ...
  Future<void> loadVendorCategories(String vendorId) async {
    try {
      ...
      final categories = await _firestoreService.getPublishedCategories(martId: vendorId);
      martCategories.clear();
  ...
  void selectCategory(String categoryId) {
    ...
    currentCategory.value = martCategories.firstWhereOrNull((c) => c.id == categoryId);
  ...
  Future<void> loadFeaturedCategories() async {
    try {
      final categories = await _firestoreService.getFeaturedCategories(
        martId: selectedVendorId.value.isNotEmpty ? selectedVendorId.value : null,
      );
  ...
  Future<void> loadCategoryItems(String categoryId) async {
    try {
      ...
      print('[MART CONTROLLER] 📂 loadCategoryItems() called for category: $categoryId');
      ...
      print('[MART CONTROLLER] ✅ Loaded ${items.length} items for category $categoryId');
      ...
      print('[MART CONTROLLER] ✅ Category items loading completed successfully');
  ...
  Future<void> loadSpotlightItems() async {
    try {
      ...
      spotlightItems.addAll(featuredItems.map((item) => {
        'id': item.id,
  ...
  Future<void> loadStealsItems() async {
    try {
      ...
      await loadItemsOnSale();
      stealsItems.clear();
      stealsItems.addAll(itemsOnSale.map((item) => {
        'id': item.id,
        'name': item.displayName,
        'image': item.mainImage,
        'price': item.currentPrice,
  ...
  void addToCart(MartItemModel item, {int quantity = 1}) {
    ...
    print('[MART] Added ${item.displayName} to cart. Total items: ${cartItems.length}');
  }
  
  /// Remove from cart (legacy method)
  void removeFromCart(String itemId) {
    cartItems.removeWhere((item) => item.id == itemId);
    print('[MART] Removed item $itemId from cart. Total items: ${cartItems.length}');
  }
  ...
  String get currentVendorName => currentVendor.value?.displayName ?? 'All Marts';
  
  /// Get current category name
  String get currentCategoryName => currentCategory.value?.displayName ?? 'All Categories';
  ...
  Future<void> retryVendorLoading() async {
    try {
      ...
      if (martVendors.isNotEmpty) {
        print('[MART CONTROLLER] ✅ Vendor retry successful: ${martVendors.length} vendors found');
        // Load additional data if vendors are now available
        await loadFeaturedItems();
        await loadItemsOnSale();
      } else {
        print('[MART CONTROLLER] ⚠️ Vendor retry completed but no vendors found');
      }
  ...
  Future<void> loadMartCategoriesEnhanced({
    ...
    try {
      ...
      print('[MART CONTROLLER] 📋 Parameters: refresh=$refresh, martId=$martId, publish=$publish');
      print('[MART CONTROLLER] 📋 Parameters: showInHomepage=$showInHomepage, hasSubcategories=$hasSubcategories');
      print('[MART CONTROLLER] 📋 Parameters: search=$search, sortBy=$sortBy, sortOrder=$sortOrder');
      ...
      if (search != null && search.isNotEmpty) {
        ...
        try {
          final uri = Uri.parse('https://jippymart.in/api/search/categories').replace(
            queryParameters: {
              'q': search,
          ...
          if (response.statusCode == 200) {
            ...
            if (data['success'] == true) {
              ...
              categories = categoriesList.map((cat) => MartCategoryModel.fromJson(cat)).toList();
              print('[MART CONTROLLER] ✅ API search returned ${categories.length} categories');
            } else {
              categories = [];
              print('[MART CONTROLLER] ⚠️ API returned success=false: ${data['message']}');
            }
          } else {
            categories = [];
            print('[MART CONTROLLER] ❌ API request failed with status: ${response.statusCode}');
          }
        } catch (e) {
          print('[MART CONTROLLER] ❌ API search failed: $e, falling back to Firestore');
  ...
  Future<void> loadHomepageCategories({int limit = 10}) async {
    try {
      ...
      print('[MART CONTROLLER] 🔍 Already loaded: ${isHomepageCategoriesLoaded.value}');
      print('[MART CONTROLLER] 🔍 Currently loading: ${isCategoryLoading.value}');
      print('[MART CONTROLLER] ==========================================');
      ...
      print('[MART CONTROLLER] 📞 Calling _martService.getHomepageCategories()...');
      final categories = await _firestoreService.getHomepageCategories(limit: limit);
      
      if (categories.isNotEmpty) {
        // Filter for categories that should be shown on homepage
        final homepageCategories = categories.where((category) => 
        ...
        if (homepageCategories.isNotEmpty) {
          ...
          print('[MART CONTROLLER] ✅ Homepage categories loaded successfully: ${homepageCategories.length} categories');
        } else {
          // If no homepage categories, use all categories
          featuredCategories.assignAll(categories);
          print('[MART CONTROLLER] ⚠️ No homepage categories found, using all categories: ${categories.length} categories');
        }
      } else {
      ...
      errorMessage.value = 'Unable to load homepage categories. Please try again later.';
  ...
  Future<void> loadTrendingItems() async {
    try {
      ...
      if (isTrendingLoading.value) {
        print('[MART CONTROLLER] ⚠️ Already loading trending items, skipping...');
        return;
      }
      ...
      print('[MART CONTROLLER] 📞 Calling _firestoreService.getTrendingItems()...');
      final items = await _firestoreService.getTrendingItems();
      
      if (items.isNotEmpty) {
        trendingItems.assignAll(items);
        print('[MART CONTROLLER] ✅ Trending items loaded successfully: ${items.length} items');
        
        // Log the isTrending status of loaded items
        final trendingCount = items.where((item) => item.isTrending == true).length;
        print('[MART CONTROLLER] 📊 Loaded items breakdown: ${trendingCount} trending out of ${items.length} total');
      } else {
        trendingItems.clear();
      ...
      isTrendingLoading.value = false;
      errorMessage.value = 'Unable to load trending items. Please try again later.';
  ...
  Future<Map<String, dynamic>> performComprehensiveSearch(String query) async {
    try {
      ...
      if (query.length < 2) {
        print('[MART CONTROLLER] ⚠️ Search query too short (minimum 2 characters)');
        return {
          'categories': <MartCategoryModel>[],
          'items': <MartItemModel>[],
          'subcategories': <MartSubcategoryModel>[],
  ...
  Future<void> searchCategories(String query) async {
    try {
      ...
      print('[MART CONTROLLER] 📞 Calling _firestoreService.searchCategories()...');
      ...
      if (categories.isNotEmpty) {
        martCategories.assignAll(categories);
        print('[MART CONTROLLER] ✅ Search results loaded: ${categories.length} categories');
      } else {
        martCategories.clear();
  ...
  Future<void> loadCategoriesWithSubcategories({
    ...
    try {
      ...
      print('[MART CONTROLLER] 📋 Parameters: publish=$publish, page=$page, limit=$limit');
      print('[MART CONTROLLER] ==========================================');
      
      isCategoryLoading.value = true;
      errorMessage.value = '';
      
      print('[MART CONTROLLER] 📞 Calling _firestoreService.getCategoriesWithSubcategories()...');
      ...
      if (categories.isNotEmpty) {
        ...
        print('[MART CONTROLLER] ✅ Categories with subcategories loaded: ${categories.length} categories');
      } else {
      ...
      isCategoryLoading.value = false;
      errorMessage.value = 'Unable to load categories. Please try again later.';
      print('[MART CONTROLLER] ❌ Error loading categories with subcategories: $e');
    }
  }
  ...
  Future<void> loadMoreCategories({
    ...
    try {
      ...
      errorMessage.value = 'Unable to load more categories. Please try again later.';
    }
  }
  ...
  void filterCategories({
    ...
    try {
      ...
      print('[MART CONTROLLER] 📋 Parameters: searchQuery=$searchQuery, publish=$publish');
      print('[MART CONTROLLER] 📋 Parameters: showInHomepage=$showInHomepage, hasSubcategories=$hasSubcategories');
      print('[MART CONTROLLER] 📋 Parameters: sortBy=$sortBy, sortOrder=$sortOrder');
      print('[MART CONTROLLER] ==========================================');
      ...
      errorMessage.value = 'Unable to filter categories. Please try again later.';
    }
  }
  ...
  Future<MartCategoryModel?> getCategoryByIdFromAPI(String categoryId) async {
    try {
      ...
      print('[MART CONTROLLER] 📞 Calling _firestoreService.getCategoryById()...');
      final category = await _firestoreService.getCategoryById(categoryId);
      
      if (category != null) {
        print('[MART CONTROLLER] ✅ Category loaded successfully: ${category.title}');
        return category;
      } else {
  ...
  Future<void> loadHomepageCategoriesStreaming({int limit = 10}) async {
    try {
      print('[MART CONTROLLER] 🏠 Streaming: Loading homepage categories from Firestore...');
      isCategoryLoading.value = true;
      
      // Try Firestore first (fastest path)
      try {
        print('[MART CONTROLLER] 🔥 Calling Firestore service for homepage categories...');
        final categories = await _firestoreService.getHomepageCategories(limit: limit);
        ...
        if (categories.isNotEmpty) {
          ...
          print('[MART CONTROLLER] ✅ Streaming: Homepage categories loaded from Firestore (${categories.length})');
          isCategoryLoading.value = false;
          isHomepageCategoriesLoaded.value = true;
          return;
        } else {
        ...
        print('[MART CONTROLLER] ❌ Firestore failed: $e, trying API fallback...');
      }
      
      // No API fallback - Firestore only
      print('[MART CONTROLLER] ❌ Firestore failed, no API fallback available');
      errorMessage.value = 'Unable to load categories from Firestore. Please check your connection.';
      isCategoryLoading.value = false;
      isHomepageCategoriesLoaded.value = false;
    } catch (e) {
      print('[MART CONTROLLER] ❌ Streaming: Error loading homepage categories: $e');
      isCategoryLoading.value = false;
    }
  }
  ...
  Future<void> loadTrendingItemsStreaming() async {
    try {
      print('[MART CONTROLLER] 🔥 Streaming: Loading trending items from Firestore...');
      isTrendingLoading.value = true;
      
      // Try to get trending items from existing data first (fastest)
      if (martItems.isNotEmpty) {
        print('[MART CONTROLLER] 🚀 Fast path: Filtering trending items from existing data');
        final trendingFromExisting = martItems.where((item) => item.isTrending == true).toList();
        if (trendingFromExisting.isNotEmpty) {
          trendingItems.clear();
          trendingItems.addAll(trendingFromExisting);
          print('[MART CONTROLLER] ✅ Fast path: Found ${trendingFromExisting.length} trending items from existing data');
          isTrendingLoading.value = false;
          return;
        }
      }
      ...
      try {
        ...
        if (items.isNotEmpty) {
          ...
          print('[MART CONTROLLER] ✅ Firestore: Trending items loaded (${items.length})');
        } else {
          // Fallback: Load all items and filter for trending
          print('[MART CONTROLLER] 🔄 Fallback: Loading all items and filtering for trending...');
  ...
  Future<void> _loadAllItemsAndFilterTrending() async {
    try {
      ...
      final trendingFromAll = allItems.where((item) => item.isTrending == true).toList();
      
      // Update trending items
      trendingItems.clear();
      trendingItems.addAll(trendingFromAll);
      
      print('[MART CONTROLLER] ✅ Fallback: Found ${trendingFromAll.length} trending items from all items');
    } catch (e) {
  ...
  Future<bool> _testTrendingEndpoint() async {
    try {
      print('[MART CONTROLLER] 🔍 Testing trending endpoint: Firestore collection mart_items');
      
      // Use the service to test the endpoint instead of direct HTTP call
      try {
        final items = await _firestoreService.getTrendingItems();
        print('[MART CONTROLLER] ✅ Trending endpoint is working - returned ${items.length} items');
        return true;
      } catch (e) {
        if (e.toString().contains('timeout') || e.toString().contains('slow')) {
          print('[MART CONTROLLER] ⏰ Trending endpoint is slow - will use fallback');
          return false;
        } else {
  ...
  Future<void> _preloadItemsForTrending() async {
    try {
      ...
      print('[MART CONTROLLER] ✅ Preloaded ${allItems.length} items for trending fallback');
      
      // If we have items, check if any are trending
      final trendingFromPreloaded = allItems.where((item) => item.isTrending == true).toList();
      if (trendingFromPreloaded.isNotEmpty && trendingItems.isEmpty) {
        print('[MART CONTROLLER] 🎯 Found ${trendingFromPreloaded.length} trending items from preloaded data');
        trendingItems.clear();
  ...
  Future<void> loadFeaturedItemsStreaming() async {
    try {
      print('[MART CONTROLLER] ⭐ Streaming: Loading featured items from Firestore...');
      isProductLoading.value = true;
      ...
      print('[MART CONTROLLER] ✅ Streaming: Featured items loaded from Firestore (${items.length})');
      isProductLoading.value = false;
    } catch (e) {
      print('[MART CONTROLLER] ❌ Streaming: Error loading featured items from Firestore: $e');
      // No API fallback - Firestore only
  ...
  Future<void> loadCategoriesStreaming() async {
    try {
      print('[MART CONTROLLER] 📂 Streaming: Loading all categories from Firestore...');
      ...
      try {
        ...
        if (categories.isNotEmpty) {
          ...
          print('[MART CONTROLLER] ✅ Streaming: All categories loaded from Firestore (${categories.length})');
          
          // Load subcategories for categories that have them
          await _loadSubcategoriesStreaming();
          return;
        } else {
          print('[MART CONTROLLER] ⚠️ No categories from Firestore, trying API fallback...');
        }
      } catch (e) {
        print('[MART CONTROLLER] ❌ Firestore failed: $e, trying API fallback...');
      }
      ...
      for (final category in martCategories) {
        if (category.hasSubcategories == true) {
          try {
            ...
            print('[MART CONTROLLER] ✅ Streaming: Subcategories loaded for ${category.title} (${subcategories.length})');
          } catch (e) {
            print('[MART CONTROLLER] ⚠️ Streaming: Error loading subcategories for ${category.title}: $e');
          }
        }
      }
  ...
  Future<void> loadSubcategoriesStreaming(String categoryId) async {
    try {
      print('[MART CONTROLLER] 📋 Loading subcategories for category: $categoryId');
      isSubcategoryLoading.value = true;
      ...
      print('[MART CONTROLLER] ✅ Loaded ${subcategories.length} subcategories for category: $categoryId');
      
    } catch (e) {
      print('[MART CONTROLLER] ❌ Error loading subcategories: $e');
      errorMessage.value = 'Unable to load subcategories. Please try again later.';
    } finally {
      isSubcategoryLoading.value = false;
    }
  }
  ...
  Future<void> _loadVendorsStreaming() async {
    try {
      ...
      if (vendors.isNotEmpty) {
        ...
        if (selectedVendorId.value.isEmpty) {
          selectVendor(vendors.first.id!);
          print('[MART CONTROLLER] 🎯 Streaming: Auto-selected first vendor: ${vendors.first.name}');
          
          // Load additional data now that we have a vendor
          await _loadAdditionalDataStreaming();
        }
        
        print('[MART CONTROLLER] ✅ Streaming: Vendors loaded (${vendors.length})');
      } else {
  ...
  Future<void> _loadAdditionalDataStreaming() async {
    try {
      ...
      print('[MART CONTROLLER] ⚠️ Streaming: Error loading additional data: $e');
    }
  }
  ...
  Future<void> _loadItemsOnSaleStreaming() async {
    try {
      ...
      print('[MART CONTROLLER] ✅ Streaming: Items on sale loaded (${saleItems.length})');
    } catch (e) {
  ...
  Stream<List<MartItemModel>> streamSimilarProducts({
    ...
    try {
      print('[MART CONTROLLER] 📡 Starting similar products stream for category: $categoryId');
  ...
  Stream<List<MartBannerModel>> streamBannersByPosition(String position, {int limit = 10}) {
    try {
      ...
      print('[MART CONTROLLER] ❌ Error creating banner stream for position $position: $e');
  ...
  Future<List<MartBannerModel>> getBannersByPosition(String position, {int limit = 10}) async {
    try {
      ...
      return await _firestoreService.getBannersByPosition(position, limit: limit);
    } catch (e) {
      print('[MART CONTROLLER] ❌ Error fetching banners for position $position: $e');
  ...
  void handleBannerTap(MartBannerModel banner) {
    try {
      print('[MART CONTROLLER] 🎯 Banner tapped: ${banner.title} (${banner.redirectType})');
      
      switch (banner.redirectType?.toLowerCase()) {
        case 'product':
          if (banner.productId != null && banner.productId!.isNotEmpty) {
            // Navigate to product details
            print('[MART CONTROLLER] 🎯 Navigating to product: ${banner.productId}');
        ...
        case 'category':
          if (banner.storeId != null && banner.storeId!.isNotEmpty) {
            ...
            print('[MART CONTROLLER] 🎯 Navigating to category: ${banner.storeId}');
        ...
        case 'external':
          if (banner.externalLink != null && banner.externalLink!.isNotEmpty) {
            // Open external link
            print('[MART CONTROLLER] 🎯 Opening external link: ${banner.externalLink}');
        ...
        default:
          print('[MART CONTROLLER] ⚠️ Unknown redirect type: ${banner.redirectType}');
      }
    } catch (e) {
  ...
  Future<void> fetchDeliverySettings() async {
    try {
      print('[MART CONTROLLER] 🚚 Fetching delivery settings from Firestore (DEPRECATED)');
      ...
      if (doc.exists) {
        ...
        deliverySettings.value = MartDeliverySettingsModel(
          freeDeliveryThreshold: (data['item_total_threshold'] as num?)?.toDouble() ?? 99.0,
          deliveryPromotionText: data['delivery_promotion_text'] ?? 'Daily',
          isActive: data['is_active'] ?? true,
          minOrderValue: (data['item_total_threshold'] as num?)?.toDouble() ?? 99.0,
          minOrderMessage: data['min_order_message'] ?? 'Min Item value is ₹99',
        );
        
        print('[MART CONTROLLER] 🚚 Delivery settings loaded from martDeliveryCharge:');
        print('  - Free delivery threshold: ₹${deliverySettings.value?.freeDeliveryThreshold}');
        print('  - Promotion text: ${deliverySettings.value?.deliveryPromotionText}');
        print('  - Is active: ${deliverySettings.value?.isActive}');
      } else {
        print('[MART CONTROLLER] ⚠️ martDeliveryCharge document not found, using defaults');
        // Set default values
        deliverySettings.value = MartDeliverySettingsModel(
  ...
  String get minOrderMessage {
    return deliverySettings.value?.minOrderMessage ?? 'Minimum order value is ₹99. Please add more items to your cart.';
  }
  ...
  String get maintenanceMessage {
    return deliverySettings.value?.maintenanceMessage ?? 'App is under maintenance. Please try again later.';
  }
  ...
  void _preloadSections() {
    ...
    print('[MART CONTROLLER] 📊 Controller state: availableSections=${availableSections.length}');
  ...
  Future<void> _loadSectionsInParallel() async {
    try {
      print('[MART CONTROLLER] ==========================================');
      print('[MART CONTROLLER] 📂 _loadSectionsInParallel() called - TRUE PARALLEL LOADING');
      print('[MART CONTROLLER] ==========================================');
      ...
      print('[MART CONTROLLER] 📂 Found ${sections.length} unique sections: $sections');
      ...
      if (sections.isNotEmpty) {
        ...
        final productFutures = sections.map((section) => _loadProductsForSectionAsync(section));
        
        // Don't wait for all products - let them load in background
        Future.wait(productFutures).then((_) {
          print('[MART CONTROLLER] ✅ All section products loaded in parallel: ${sections.length} sections');
        ...
        print('[MART CONTROLLER] ✅ Sections loaded immediately: ${sections.length} sections');
      } else {
  ...
  Future<void> _loadSectionsProgressively() async {
    try {
      print('[MART CONTROLLER] ==========================================');
      print('[MART CONTROLLER] 📂 _loadSectionsProgressively() called - SILENT STREAMING');
      print('[MART CONTROLLER] ==========================================');
      ...
      if (sections.isNotEmpty) {
        ...
        final futures = sections.map((section) => _loadProductsForSectionAsync(section));
        await Future.wait(futures);
        
        print('[MART CONTROLLER] ✅ Sections loaded silently: ${sections.length} sections');
      } else {
        print('[MART CONTROLLER] ⚠️ No sections found');
      }
  ...
  Future<void> _loadProductsForSectionAsync(String section) async {
    try {
      ...
      print('[MART CONTROLLER] ✅ Loaded ${products.length} products for section: $section');
      
    } catch (e) {
      print('[MART CONTROLLER] ❌ Error loading products for section $section: $e');
  ...
  Stream<List<MartItemModel>> streamProductsByBrand(String brandID) {
    try {
      ...
      return _firestoreService.streamItemsByBrand(brandID).map((products) {
        print('[MART CONTROLLER] 📦 Received ${products.length} products for brand: $brandID');
        return products;
      });
    } catch (e) {
  ...
  Future<void> loadSectionsImmediately() async {
    try {
      ...
      if (_sectionsLoadingTriggered) {
        print('[MART CONTROLLER] ⚠️ Sections loading already triggered, skipping...');
        return;
      }
  ...
  void addTestSections() {
    ...
    if (availableSections.isNotEmpty) {
      print('[MART CONTROLLER] 🧪 Sections already loaded, skipping test sections');
      return;
    }
    
    print('[MART CONTROLLER] 🧪 Adding test sections for debugging...');
    availableSections.clear();
    availableSections.addAll(['Pet Care', 'General', 'Essentials & Daily Needs']);
    print('[MART CONTROLLER] 🧪 Test sections added: ${availableSections.length} sections');
