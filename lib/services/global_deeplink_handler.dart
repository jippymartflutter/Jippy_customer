...
class GlobalDeeplinkHandler {
  ...
  static final GlobalDeeplinkHandler _instance = GlobalDeeplinkHandler._privateConstructor();
  ...
  void storeDeeplink(String url) {
    ...
    if (_isProcessing) {
      print('🔗 [GLOBAL_DEEPLINK] ⚠️ Already processing a deep link, queuing this one...');
    ...
    _isProcessing = true;
    print('🔗 [GLOBAL_DEEPLINK] ✅ Set processing flag to prevent concurrent processing');
    
    _pendingDeeplink = url;
    print('🔗 [GLOBAL_DEEPLINK] ✅ Deeplink stored. Has pending: ${hasPendingDeeplink}');
    print('🔗 [GLOBAL_DEEPLINK] ✅ Pending deeplink value: $_pendingDeeplink');
    
    // **ENHANCED PROCESSING: Use crash prevention for ALL deep links**
    print('🔗 [GLOBAL_DEEPLINK] 🛡️ Using enhanced crash prevention for all deep links');
  ...
  bool get hasPendingDeeplink => _pendingDeeplink != null && _pendingDeeplink!.isNotEmpty;
  ...
  void navigatePendingDeeplink() {
    ...
    print('🔗 [GLOBAL_DEEPLINK] 🚀 Navigating to pending deeplink: $_pendingDeeplink');
    _isProcessing = true;
  ...
  void navigateToLink(String link) {
    ...
    print('🔗 [GLOBAL_DEEPLINK] Parsed URI: scheme=${uri.scheme}, host=${uri.host}, path=${uri.path}');
    ...
    print('🔗 [GLOBAL_DEEPLINK] Final path: $pathToCheck, segments: $pathSegments');
    ...
    if (pathSegments.isNotEmpty && pathSegments[0] == 'product') {
      ...
      print('🔗 [GLOBAL_DEEPLINK] 🍽️ Navigating to Restaurant ID: $restaurantId');
  ...
  void _navigateToProduct(String productId) async {
    try {
      ...
      if (!Get.isRegistered<CartController>()) {
        print('🔗 [GLOBAL_DEEPLINK] CartController not available, initializing...');
        Get.put(CartController(), permanent: true);
      }
      
      print('🔗 [GLOBAL_DEEPLINK] Calling martController.getProductById($productId)...');
      ...
      if (product != null) {
        ...
        print('🔗 [GLOBAL_DEEPLINK] DEBUG - Waiting briefly for home screen to load before navigation...');
        await Future.delayed(Duration(milliseconds: 500));
        
        // Use GetX navigation instead of navigator key
        print('🔗 [GLOBAL_DEEPLINK] DEBUG - Using GetX navigation to product details...');
        Get.to(() => MartProductDetailsScreen(product: product));
        print('🔗 [GLOBAL_DEEPLINK] ✅ Successfully navigated to product details using GetX');
      } else {
  ...
  void _navigateToRestaurant(String restaurantId) async {
    try {
      ...
      if (_currentRestaurantId == restaurantId) {
        print('🔗 [GLOBAL_DEEPLINK] ⚠️ Same restaurant already loaded, skipping: $restaurantId');
        return;
      }
      ...
      print('🔗 [GLOBAL_DEEPLINK] 🔍 Fetching restaurant data for ID: $restaurantId');
      ...
      if (restaurant != null) {
        ...
        print('🔗 [GLOBAL_DEEPLINK] Restaurant Status: ${restaurant.isOpen == true ? "OPEN" : "CLOSED"}');
        
        // **FIXED: Minimal delay for faster navigation**
        print('🔗 [GLOBAL_DEEPLINK] DEBUG - Minimal delay for faster navigation...');
        await Future.delayed(Duration(milliseconds: 100));
        
        // **FIXED: Use GetX navigation with restaurant data and allow override**
        print('🔗 [GLOBAL_DEEPLINK] DEBUG - Using GetX navigation to restaurant details with data...');
        
        // **FIXED: Use Get.to() instead of Get.offAll() to preserve navigation stack**
        // This allows the back button to work properly
        print('🔗 [GLOBAL_DEEPLINK] DEBUG - Using Get.to() to preserve navigation stack...');
        ...
        try {
          if (Get.isRegistered<RestaurantDetailsController>()) {
            print('🔗 [GLOBAL_DEEPLINK] 🔍 Controller is registered, attempting update...');
            final controller = Get.find<RestaurantDetailsController>();
            print('🔗 [GLOBAL_DEEPLINK] 🔍 Controller found, calling updateRestaurant...');
            controller.updateRestaurant(restaurant);
            print('🔗 [GLOBAL_DEEPLINK] ✅ Updated controller with new restaurant data: ${restaurant.title}');
          } else {
        ...
        print('🔗 [GLOBAL_DEEPLINK] ✅ Successfully navigated to restaurant details with data: ${restaurant.title}');
      } else {
  ...
  void testHandler() {
    print('🔗 [GLOBAL_DEEPLINK] ✅ Handler is working! Test method called successfully');
  }

  /// Process deep link with enhanced crash prevention
  Future<void> _processDeeplink(String url) async {
    try {
      print('🔗 [GLOBAL_DEEPLINK] Processing deep link with enhanced crash prevention: $url');
      ...
      if (url.contains('/restaurant/') || url.contains('/restaurants/')) {
        ...
        if (restaurantId != null) {
          print('🔗 [GLOBAL_DEEPLINK] 🍽️ Processing restaurant deep link: $restaurantId');
          _navigateToRestaurant(restaurantId);
        }
      ...
      print('🔗 [GLOBAL_DEEPLINK] 🛡️ Graceful error handling - app continues running');
    }
  }
  ...
  Future<void> _navigateToCategory(String categoryId) async {
    try {
      ...
      print('🔗 [GLOBAL_DEEPLINK] 🔍 Fetching category data for ID: $categoryId');
      ...
      if (category != null) {
        ...
        print('🔗 [GLOBAL_DEEPLINK] Category Status: ${category.publish == true ? "PUBLISHED" : "UNPUBLISHED"}');
        ...
        print('🔗 [GLOBAL_DEEPLINK] Navigating to specific category detail screen...');
        Get.to(() => const MartCategoryDetailScreen(), arguments: {
          'categoryId': categoryId,
          'categoryName': category.title ?? 'Category', // Use actual category title
        });
        print('🔗 [GLOBAL_DEEPLINK] ✅ Successfully navigated to specific category detail screen!');
      } else {
