...
class FinalDeepLinkService {
  // Singleton pattern
  static final FinalDeepLinkService _instance = FinalDeepLinkService._internal();
  ...
  static const MethodChannel _methodChannel = MethodChannel('deep_link_methods');
  ...
  Future<void> init(GlobalKey<NavigatorState> navigatorKey) async {
    if (_initialized) {
      ...
      print('🔗 [FLUTTER] PRINT TEST - DeepLinkService already initialized, skipping...');
    ...
    print('🚀 [FLUTTER] PRINT TEST - DeepLinkService INIT started - Singleton pattern');
    ...
    _sub = _eventChannel.receiveBroadcastStream().listen((dynamic link) {
      if (link != null) {
        ...
        print('🔗 [FLUTTER] PRINT TEST - Received link from Android (event): $url');
        _handleLink(url);
      }
    }, onError: (error) {
  ...
  Future<void> _getInitialLink() async {
    try {
      ...
      final String? initial = await _methodChannel.invokeMethod<String>('getInitialLink');
      if (initial != null) {
  ...
  void _handleLink(String url) async {
    ...
    if (_hasProcessedDeepLink) {
      print('🔥🔥🔥 [FLUTTER] Deep link already processed, ignoring duplicate: $url');
      return;
    }
    ...
    print('🔥🔥🔥 [FLUTTER] Storing deep link for processing after login: $url');
    _pendingDeepLink = url;
    print('🔥🔥🔥 [FLUTTER] Deep link stored successfully. _pendingDeepLink: $_pendingDeepLink');
    
    // Check if user is already logged in
    final isLoggedIn = await _checkUserLoginStatus();
    if (isLoggedIn) {
      print('🔥🔥🔥 [FLUTTER] User is logged in, processing deep link immediately');
      
      // **FIXED: Call GlobalDeeplinkHandler.storeDeeplink() to process the deep link**
      print('🔥🔥🔥 [FLUTTER] Calling GlobalDeeplinkHandler.storeDeeplink() with URL: $url');
      
      // **SIMPLE TEST: Direct call to GlobalDeeplinkHandler**
      try {
        print('🔥🔥🔥 [FLUTTER] TEST: About to call GlobalDeeplinkHandler.storeDeeplink()');
        GlobalDeeplinkHandler.instance.storeDeeplink(url);
        print('🔥🔥🔥 [FLUTTER] TEST: ✅ Successfully called GlobalDeeplinkHandler.storeDeeplink()');
      } catch (e) {
        print('🔥🔥🔥 [FLUTTER] TEST: ❌ Error calling GlobalDeeplinkHandler.storeDeeplink(): $e');
      }
      
      _newSimpleDeepLinkHandler(url);
    } else {
      print('🔥🔥🔥 [FLUTTER] User not logged in, deep link will be processed after login');
    }
  }
  ...
  void processPendingDeepLinkAfterLogin() {
    ...
    print('🔥🔥🔥 [FLUTTER] _pendingDeepLink != null: ${_pendingDeepLink != null}');
    print('🔥🔥🔥 [FLUTTER] _pendingDeepLink!.isNotEmpty: ${_pendingDeepLink?.isNotEmpty ?? false}');
    
    if (_pendingDeepLink != null && _pendingDeepLink!.isNotEmpty) {
      print('🔥🔥🔥 [FLUTTER] Processing pending deep link after login: $_pendingDeepLink');
      
      // **FIXED: Call GlobalDeeplinkHandler.storeDeeplink() to process the pending deep link**
      print('🔥🔥🔥 [FLUTTER] Calling GlobalDeeplinkHandler.storeDeeplink() with pending URL: $_pendingDeepLink');
  ...
  void _navigateToProduct(String productId) async {
    try {
      ...
      print('🔍 [FLUTTER] PRINT TEST - Loading product details for: $productId');
      ...
      if (!Get.isRegistered<MartController>()) {
        ...
        print('⚠️ [FLUTTER] PRINT TEST - MartController not available, navigating to mart home');
        _navigateToMartHome();
        return;
      }
      ...
      if (!Get.isRegistered<CartController>()) {
        ...
        print('⚠️ [FLUTTER] PRINT TEST - CartController not available, initializing...');
      ...
      if (product != null) {
        ...
        print('🔍 [FLUTTER] DEBUG - Navigator key state: ${GlobalDeeplinkHandler.navigatorKey.currentState}');
        print('🔍 [FLUTTER] DEBUG - Navigator key mounted: ${GlobalDeeplinkHandler.navigatorKey.currentState?.mounted}');
        
        // Use GetX navigation instead of navigator key
        print('🔍 [FLUTTER] DEBUG - Using GetX navigation to product details...');
        try {
          Get.to(() => MartProductDetailsScreen(product: product));
          log('🎯 [FLUTTER] Successfully navigated to product details using GetX');
          print('🎯 [FLUTTER] PRINT TEST - Successfully navigated to product details using GetX');
        } catch (e) {
          ...
          Future.delayed(Duration(seconds: 2), () {
            if (GlobalDeeplinkHandler.navigatorKey.currentState != null) {
              print('🔍 [FLUTTER] DEBUG - Fallback: Navigator key available, attempting navigation...');
              GlobalDeeplinkHandler.navigatorKey.currentState?.push(
                MaterialPageRoute(
                  builder: (context) => MartProductDetailsScreen(product: product),
                ),
              );
              print('✅ [FLUTTER] PRINT TEST - Successfully navigated to product details (fallback)');
            } else {
              print('❌ [FLUTTER] DEBUG - Fallback failed: Navigator state still null');
            }
          });
        }
      } else {
  ...
  void _navigateToRestaurantWithData(String restaurantId) async {
    try {
      ...
      if (restaurant != null) {
        ...
        print('🔥 [NEW HANDLER] Restaurant Status: ${restaurant.isOpen == true ? "OPEN" : "CLOSED"}');
        ...
        print('🔥✅ [NEW HANDLER] Successfully navigated to restaurant details with data!');
      } else {
  ...
  void _navigateToCategoryWithData(String categoryId) async {
    try {
      ...
      if (category != null) {
        ...
        print('🔥 [NEW HANDLER] Category Status: ${category.publish == true ? "PUBLISHED" : "UNPUBLISHED"}');
        ...
        print('🔥 [NEW HANDLER] Navigating to specific category detail screen...');
        Get.to(() => const MartCategoryDetailScreen(), arguments: {
          'categoryId': categoryId,
          'categoryName': category.title ?? 'Category', // Use actual category title
        });
        print('🔥✅ [NEW HANDLER] Successfully navigated to specific category detail screen!');
      } else {
  ...
  void _navigateToMartHome() async {
    try {
      ...
      print('📍 [DEEP_LINK_SERVICE] Current Zone: ${Constant.selectedZone?.id ?? "NULL"} (${Constant.selectedZone?.name ?? "NULL"})');
      print('📍 [DEEP_LINK_SERVICE] User Location: ${Constant.selectedLocation.location?.latitude ?? "NULL"}, ${Constant.selectedLocation.location?.longitude ?? "NULL"}');
      
      // Check if mart is available in current zone
      final isMartAvailable = await MartZoneUtils.isMartAvailableInCurrentZone();
      
      if (isMartAvailable) {
        print('✅ [DEEP_LINK_SERVICE] Mart is available - Navigating to MartNavigationScreen');
        print('🎯 [DEEP_LINK_SERVICE] Navigation: Deep Link -> MartNavigationScreen');
        ...
        print('✅ [DEEP_LINK_SERVICE] Deep link navigation completed successfully');
      } else {
        print('❌ [DEEP_LINK_SERVICE] Mart not available - Redirecting to dashboard');
        // Show coming soon message for zones without mart
        log('⚠️ [FLUTTER] Mart not available in current zone: ${Constant.selectedZone?.id}');
        print('⚠️ [FLUTTER] PRINT TEST - Mart not available in current zone: ${Constant.selectedZone?.id}');
        print('🎯 [DEEP_LINK_SERVICE] Navigation: Deep Link -> Dashboard (fallback)');
        _navigateToDashboard();
      }
      
      print('🔗 [DEEP_LINK_SERVICE] ===== DEEP LINK MART NAVIGATION COMPLETED =====\n');
    } catch (e) {
      ...
      print('🎯 [DEEP_LINK_SERVICE] Navigation: Deep Link -> Dashboard (error fallback)');
      _navigateToDashboard();
      print('🔗 [DEEP_LINK_SERVICE] ===== DEEP LINK MART NAVIGATION COMPLETED (ERROR) =====\n');
    }
  }
  ...
  void _waitForNavigator() {
    ...
    Timer.periodic(Duration(milliseconds: 100), (timer) {
      if (GlobalDeeplinkHandler.navigatorKey.currentState != null) {
        timer.cancel();
        print('🔗 [FLUTTER] Navigator is now available, processing pending deep link...');
  ...
  void _processPendingDeepLink() {
    ...
    if (_hasProcessedDeepLink) {
      print('🔗 [FLUTTER] Deep link already processed — skipping duplicate processing');
  ...
  void _tryProcessLink(String url) {
    ...
    try {
      ...
      print('⏳ [FLUTTER] DashboardController not ready yet, retrying in 500ms...');
    ...
    print('🔗 [FLUTTER] Dashboard controller found, proceeding with navigation...');
  ...
  void _simpleNavigateToProduct(String productId) async {
    ...
    print('🔍 [FLUTTER] DEBUG - Waiting 5 seconds for everything to be ready...');
    ...
    try {
      if (GlobalDeeplinkHandler.navigatorKey.currentState != null) {
        print('🔍 [FLUTTER] DEBUG - Navigator available, pushing SIMPLE route...');
        ...
        print('✅ [FLUTTER] PRINT TEST - Successfully navigated to SIMPLE product screen');
      } else {
  ...
  void _newSimpleDeepLinkHandler(String url) async {
    ...
    int attempts = 0;
    while (GlobalDeeplinkHandler.navigatorKey.currentState == null && attempts < 100) {
    ...
    print('🔥 [NEW HANDLER] Navigator ready, waiting 10 seconds for app to load...');
    await Future.delayed(Duration(seconds: 5));
    ...
    if (uri.scheme == 'jippymart') {
      ...
      if (pathSegments.length >= 2 && pathSegments[0] == 'product') {
        ...
        final restaurantId = pathSegments[1];
        print('🔥 [NEW HANDLER] HTTPS scheme (plural) - Restaurant ID: $restaurantId');
        _navigateToRestaurantWithData(restaurantId);
        return;
        ...
        final categoryId = pathSegments[1];
        print('🔥 [NEW HANDLER] HTTPS scheme (plural) - Category ID: $categoryId');
        _navigateToCategoryWithData(categoryId);
        return;
    ...
    if (productId != null) {
      ...
      print('🔥 [NEW HANDLER] Navigating to REAL product details screen with actual data...');
      try {
        ...
        try {
          ...
          // Ensure CartController is available for product details screen
          if (!Get.isRegistered<CartController>()) {
            print('🔥 [NEW HANDLER] CartController not available, initializing for deep link...');
            Get.put(CartController(), permanent: true);
          }
          
          // Fetch the real product data
          print('🔥 [NEW HANDLER] Calling martController.getProductById($productId)...');
          final product = await martController.getProductById(productId);
          ...
          if (product != null) {
            ...
            print('🔥✅ [NEW HANDLER] Successfully navigated to REAL product details screen with actual data!');
          } else {
            ...
            print('🔍 [NEW HANDLER] Redirecting to mart home instead of showing fallback product');
        ...
        print('🔍 [NEW HANDLER] Redirecting to mart home due to navigation error');
  ...
  void _navigateToProductWithGetX(String productId) async {
    ...
    print('🔍 [FLUTTER] DEBUG - Waiting 20 seconds for everything to be ready...');
