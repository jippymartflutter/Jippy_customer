...
class MartFirestoreService extends GetxService {
  ...
  Future<List<MartItemModel>> getTrendingItems({int limit = 20}) async {
    try {
      ...
      print('[MART FIRESTORE] 🔥 Firestore query completed, found ${querySnapshot.docs.length} trending items');
      ...
      final items = querySnapshot.docs.map((doc) {
        try {
          ...
          if (data is! Map<String, dynamic>) {
            print('[MART FIRESTORE] ⚠️ Document data is not a Map for ${doc.id}, type: ${data.runtimeType}');
            return null;
          }
          ...
          if (itemData['product_specification'] == null) itemData['product_specification'] = {};
          ...
          print('[MART FIRESTORE] ❌ Error parsing Firestore document ${doc.id}: $e');
          print('[MART FIRESTORE] Document data: ${doc.data()}');
          return null;
        }
      }).whereType<MartItemModel>().toList();
      
      print('[MART FIRESTORE] ✅ Successfully parsed ${items.length} trending items from Firestore');
      
      // Debug: Log the trending items
      for (int i = 0; i < items.length; i++) {
        final item = items[i];
        print('[MART FIRESTORE]   ${i + 1}. ${item.name} - isTrending: ${item.isTrending}, price: ₹${item.price}');
      }
      
      return items;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching trending items from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartItemModel>> getFeaturedItems({int limit = 20}) async {
    try {
      ...
      print('[MART FIRESTORE] ⭐ Firestore query completed, found ${querySnapshot.docs.length} featured items');
      ...
      print('[MART FIRESTORE] ✅ Successfully parsed ${items.length} featured items from Firestore');
      
      // Debug: Log the featured items
      for (int i = 0; i < items.length; i++) {
        final item = items[i];
        print('[MART FIRESTORE]   ${i + 1}. ${item.name} - isFeature: ${item.isFeature}, price: ₹${item.price}');
      }
      
      return items;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching featured items from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartItemModel>> getItemsOnSale({int limit = 20}) async {
    try {
      ...
      print('[MART FIRESTORE] 🏷️ Firestore query completed, found ${querySnapshot.docs.length} items');
      ...
      final items = querySnapshot.docs.map((doc) {
        try {
          ...
          if (data['product_specification'] == null) data['product_specification'] = {};
      ...
      final saleItems = items.where((item) => 
        item.disPrice != null && 
        item.disPrice! < item.price
      ).toList();
      
      print('[MART FIRESTORE] ✅ Successfully found ${saleItems.length} items on sale');
      
      // Debug: Log the sale items
      for (int i = 0; i < saleItems.length; i++) {
        final item = saleItems[i];
        print('[MART FIRESTORE]   ${i + 1}. ${item.name} - Original: ₹${item.price}, Sale: ₹${item.disPrice}');
      }
      
      return saleItems;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching items on sale from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartItemModel>> searchItems({
    ...
    try {
      ...
      print('[MART FIRESTORE] 🔍 Firestore query completed, found ${querySnapshot.docs.length} items');
      ...
      print('[MART FIRESTORE] ✅ Search completed, found ${searchResults.length} matching items');
      
      // Debug: Log the search results
      for (int i = 0; i < searchResults.length; i++) {
        final item = searchResults[i];
        final shortDescription = item.description.length > 50 ? '${item.description.substring(0, 50)}...' : item.description;
  ...
  Future<List<MartCategoryModel>> getCategories({int limit = 100}) async {
    try {
      ...
      print('[MART FIRESTORE] 📂 Firestore query completed, found ${querySnapshot.docs.length} categories');
      ...
      final categories = querySnapshot.docs.map((doc) {
        try {
          ...
          // Handle array fields that might be null
          if (data['review_attributes'] == null) data['review_attributes'] = [];
          
          // Handle numeric fields that might be strings
          if (data['category_order'] is String) {
            data['category_order'] = int.tryParse(data['category_order']) ?? 0;
          }
          if (data['section_order'] is String) {
            data['section_order'] = int.tryParse(data['section_order']) ?? 0;
          }
          
          return MartCategoryModel.fromJson(data);
        } catch (e) {
          print('[MART FIRESTORE] ❌ Error parsing Firestore category document ${doc.id}: $e');
          print('[MART FIRESTORE] Category data: ${doc.data()}');
          return null;
        }
      }).whereType<MartCategoryModel>().toList();
      
      // Sort categories by category_order since we're not using orderBy
      categories.sort((a, b) => (a.categoryOrder ?? 0).compareTo(b.categoryOrder ?? 0));
      
      print('[MART FIRESTORE] ✅ Successfully parsed ${categories.length} categories from Firestore (client-side ordered)');
      ...
      for (int i = 0; i < categories.length; i++) {
        ...
        print('[MART FIRESTORE]   ${i + 1}. $title - Order: $order, Section: $section');
      }
  ...
  Future<List<MartCategoryModel>> getHomepageCategories({int limit = 10}) async {
    try {
      print('[MART FIRESTORE] 🏠 Fetching homepage categories from Firestore...');
      print('[MART FIRESTORE] 🔍 Collection: mart_categories');
      print('[MART FIRESTORE] 🔍 Query: publish=true, show_in_homepage=true, orderBy=category_order');
      print('[MART FIRESTORE] ✅ Using composite index for optimal performance!');
      
      // Test Firestore connection
      print('[MART FIRESTORE] 🔍 Testing Firestore connection...');
      final testSnapshot = await _firestore.collection('mart_categories').limit(1).get();
      print('[MART FIRESTORE] ✅ Firestore connection successful, found ${testSnapshot.docs.length} documents');
      ...
      print('[MART FIRESTORE] 🔍 Found ${querySnapshot.docs.length} total categories, ${homepageCategories.length} are homepage categories');
      ...
      if (homepageCategories.isEmpty) {
        ...
        for (int i = 0; i < querySnapshot.docs.length && i < 5; i++) {
          ...
          final data = doc.data();
          print('[MART FIRESTORE]   ${i + 1}. ${doc.id} - ${data['title']} - show_in_homepage: ${data['show_in_homepage']}');
        }
        
        // Fallback: Show first few categories even if they don't have show_in_homepage=true
        if (querySnapshot.docs.isNotEmpty) {
          print('[MART FIRESTORE] 🔄 Fallback: Using first ${querySnapshot.docs.length} categories');
          ...
          return fallbackDocs.map((doc) {
            try {
              ...
              if (categoryData['review_attributes'] == null) categoryData['review_attributes'] = [];
              if (categoryData['category_order'] is String) {
                categoryData['category_order'] = int.tryParse(categoryData['category_order']) ?? 0;
              }
              if (categoryData['section_order'] is String) {
                categoryData['section_order'] = int.tryParse(categoryData['section_order']) ?? 0;
              }
              return MartCategoryModel.fromJson(categoryData);
            } catch (e) {
              print('[MART FIRESTORE] ❌ Error parsing fallback category ${doc.id}: $e');
              return null;
            }
          }).whereType<MartCategoryModel>().toList();
        }
      ...
      print('[MART FIRESTORE] 🏠 Firestore query completed, found ${finalDocs.length} homepage categories');
      
      // Convert Firestore documents to MartCategoryModel
      final categories = finalDocs.map((doc) {
        try {
          final data = doc.data();
          final Map<String, dynamic> categoryData = Map<String, dynamic>.from(data);
          ...
          print('[MART FIRESTORE] ❌ Error parsing Firestore homepage category document ${doc.id}: $e');
          return null;
        }
      }).whereType<MartCategoryModel>().toList();
      ...
      print('[MART FIRESTORE] ✅ Successfully parsed ${categories.length} homepage categories from Firestore (client-side ordered)');
      ...
      print('[MART FIRESTORE] ❌ Error fetching homepage categories from Firestore: $e');
  ...
  Future<List<MartSubcategoryModel>> getSubcategoriesByParent({
    ...
    try {
      print('[MART FIRESTORE] 📋 Fetching subcategories for parent category: $parentCategoryId');
      ...
      print('[MART FIRESTORE] 📋 Firestore query completed, found ${querySnapshot.docs.length} subcategories');
      
      if (querySnapshot.docs.isEmpty) {
        print('[MART FIRESTORE] ⚠️ No subcategories found for parent category: $parentCategoryId');
        return [];
      }
      ...
      final subcategories = querySnapshot.docs.map((doc) {
        try {
          final data = doc.data();
          final Map<String, dynamic> subcategoryData = Map<String, dynamic>.from(data);
          
          // Add document ID to the data
          subcategoryData['id'] = doc.id;
          
          // Handle array fields that might be null
          if (subcategoryData['review_attributes'] == null) subcategoryData['review_attributes'] = [];
          
          // Handle numeric fields that might be strings
          if (subcategoryData['subcategory_order'] is String) {
            subcategoryData['subcategory_order'] = int.tryParse(subcategoryData['subcategory_order']) ?? 0;
          }
          if (subcategoryData['category_order'] is String) {
            subcategoryData['category_order'] = int.tryParse(subcategoryData['category_order']) ?? 0;
          }
          if (subcategoryData['section_order'] is String) {
            subcategoryData['section_order'] = int.tryParse(subcategoryData['section_order']) ?? 0;
          }
          
          return MartSubcategoryModel.fromJson(subcategoryData);
        } catch (e) {
          print('[MART FIRESTORE] ❌ Error parsing Firestore subcategory document ${doc.id}: $e');
          print('[MART FIRESTORE] Subcategory data: ${doc.data()}');
          return null;
        }
      }).whereType<MartSubcategoryModel>().toList();
      
      // Sort subcategories by subcategory_order since we're not using orderBy
      subcategories.sort((a, b) => (a.subcategoryOrder ?? 0).compareTo(b.subcategoryOrder ?? 0));
      
      print('[MART FIRESTORE] ✅ Successfully parsed ${subcategories.length} subcategories from Firestore (client-side ordered)');
      ...
      for (int i = 0; i < subcategories.length; i++) {
        ...
        print('[MART FIRESTORE]   ${i + 1}. $title - Order: $order, Parent: $parentId');
      }
      
      return subcategories;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching subcategories from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartSubcategoryModel>> getAllHomepageSubcategories() async {
    try {
      print('[MART FIRESTORE] 🔥 Fetching all homepage subcategories directly from Firestore...');
      ...
      try {
        ...
        print('[MART FIRESTORE] 🔥 Found ${querySnapshot.docs.length} homepage subcategories in Firestore (with ordering)');
        ...
        for (int i = 0; i < querySnapshot.docs.length; i++) {
          ...
          if (data is Map<String, dynamic>) {
            print('[MART FIRESTORE] 📋    show_in_homepage: ${data['show_in_homepage']}');
            print('[MART FIRESTORE] 📋    publish: ${data['publish']}');
            print('[MART FIRESTORE] 📋    title: ${data['title']}');
            print('[MART FIRESTORE] 📋    parent_category_id: ${data['parent_category_id']}');
          }
        ...
        final subcategories = querySnapshot.docs.map((doc) {
          try {
            ...
            final subcategory = MartSubcategoryModel.fromJson(subcategoryData);
            print('[MART FIRESTORE] ✅ Successfully parsed ${doc.id}: ${subcategory.title}');
            return subcategory;
          } catch (e) {
            print('[MART FIRESTORE] ❌ Error parsing subcategory document ${doc.id}: $e');
            print('[MART FIRESTORE] Document data: ${doc.data()}');
            return null;
          }
        }).whereType<MartSubcategoryModel>().toList();
        ...
        print('[MART FIRESTORE] ✅ Successfully parsed ${subcategories.length} homepage subcategories from Firestore');
        print('[MART FIRESTORE] 📊 PARSED SUBCATEGORIES:');
        
        // Debug: Log the homepage subcategories
        for (int i = 0; i < subcategories.length; i++) {
          final subcategory = subcategories[i];
          print('[MART FIRESTORE]   ${i + 1}. ${subcategory.title} - Parent: ${subcategory.parentCategoryTitle} - Show in Homepage: ${subcategory.showInHomepage}');
        }
        
        return subcategories;
        
      } catch (e) {
        print('[MART FIRESTORE] ⚠️ First query failed (likely index issue), trying without ordering: $e');
        ...
        final querySnapshot = await query.get();
        print('[MART FIRESTORE] 🔥 Found ${querySnapshot.docs.length} homepage subcategories in Firestore (without ordering)');
        ...
        final subcategories = querySnapshot.docs.map((doc) {
          try {
            final data = doc.data();
            if (data == null) {
              print('[MART FIRESTORE] ❌ Document ${doc.id} has null data (fallback)');
              return null;
            }
            
            // Ensure data is a Map<String, dynamic>
            if (data is! Map<String, dynamic>) {
              print('[MART FIRESTORE] ⚠️ Document data is not a Map for ${doc.id}, type: ${data.runtimeType} (fallback)');
              return null;
            }
            ...
            print('[MART FIRESTORE] 🔍 Parsing document ${doc.id} (fallback):');
            print('[MART FIRESTORE] 🔍   Raw data: $subcategoryData');
            
            final subcategory = MartSubcategoryModel.fromJson(subcategoryData);
            print('[MART FIRESTORE] ✅ Successfully parsed ${doc.id}: ${subcategory.title} (fallback)');
            return subcategory;
          } catch (e) {
            print('[MART FIRESTORE] ❌ Error parsing subcategory document ${doc.id}: $e (fallback)');
            print('[MART FIRESTORE] Document data: ${doc.data()}');
            return null;
          }
        }).whereType<MartSubcategoryModel>().toList();
        ...
        print('[MART FIRESTORE] ✅ Successfully parsed ${subcategories.length} homepage subcategories from Firestore (fallback)');
      ...
      print('[MART FIRESTORE] ❌ Error fetching homepage subcategories from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartSubcategoryModel>> getAllSubcategoriesDebug() async {
    try {
      print('[MART FIRESTORE] 🔍 DEBUG: Fetching ALL subcategories from Firestore (no filters)...');
      
      final querySnapshot = await _firestore.collection('mart_subcategories').get();
      print('[MART FIRESTORE] 🔍 DEBUG: Found ${querySnapshot.docs.length} total subcategories in Firestore');
      ...
      for (int i = 0; i < querySnapshot.docs.length; i++) {
        ...
        if (data is Map<String, dynamic>) {
          print('[MART FIRESTORE] 🔍 DEBUG:    show_in_homepage: ${data['show_in_homepage']}');
          print('[MART FIRESTORE] 🔍 DEBUG:    publish: ${data['publish']}');
          print('[MART FIRESTORE] 🔍 DEBUG:    title: ${data['title']}');
          print('[MART FIRESTORE] 🔍 DEBUG:    parent_category_id: ${data['parent_category_id']}');
        }
      ...
      final subcategories = querySnapshot.docs.map((doc) {
        try {
          ...
          // Ensure data is a Map<String, dynamic>
          if (data is! Map<String, dynamic>) {
            print('[MART FIRESTORE] 🔍 DEBUG: Document data is not a Map for ${doc.id}, type: ${data.runtimeType}');
            return null;
          }
          ...
          final subcategory = MartSubcategoryModel.fromJson(subcategoryData);
          print('[MART FIRESTORE] 🔍 DEBUG: Successfully parsed ${doc.id}: ${subcategory.title}');
          return subcategory;
        } catch (e) {
          print('[MART FIRESTORE] 🔍 DEBUG: Error parsing subcategory document ${doc.id}: $e');
          print('[MART FIRESTORE] 🔍 DEBUG: Document data: ${doc.data()}');
          return null;
        }
      }).whereType<MartSubcategoryModel>().toList();
      
      print('[MART FIRESTORE] 🔍 DEBUG: Successfully parsed ${subcategories.length} subcategories from Firestore');
      
      return subcategories;
    } catch (e) {
      print('[MART FIRESTORE] 🔍 DEBUG: Error fetching all subcategories from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartCategoryModel>> getCategoriesBySection({
    ...
    try {
      ...
      print('[MART FIRESTORE] 📂 Firestore query completed, found ${querySnapshot.docs.length} categories for section "$section"');
      ...
      print('[MART FIRESTORE] ✅ Successfully parsed ${categories.length} categories for section "$section" (server-side ordered)');
      return categories;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching categories for section "$section": $e');
      return [];
    }
  }
  ...
  Future<List<MartSubcategoryModel>> searchSubcategories({
    ...
    try {
      ...
      print('[MART FIRESTORE] ✅ Search completed, found ${searchResults.length} matching subcategories');
      ...
      for (int i = 0; i < searchResults.length; i++) {
        ...
        final shortDescription = description.length > 50 ? '${description.substring(0, 50)}...' : description;
  ...
  Future<List<MartCategoryModel>> searchCategories({
    ...
    try {
      ...
      print('[MART FIRESTORE] ✅ Search completed, found ${searchResults.length} matching categories');
  ...
  Future<List<MartItemModel>> getItemsByCategory({
    ...
    try {
      ...
      // Build the query
      Query query = _firestore.collection('mart_items').where('publish', isEqualTo: true);
      ...
      final items = querySnapshot.docs.map((doc) {
        try {
          final data = doc.data();
          if (data == null) {
            print('[MART FIRESTORE] ⚠️ Document data is null for ${doc.id}');
            return null;
          }
          ...
          if (itemData['options'] == null) itemData['options'] = [];
          if (itemData['photos'] == null) itemData['photos'] = [];
          if (itemData['review_attributes'] == null) itemData['review_attributes'] = [];
          if (itemData['subcategoryID'] == null) itemData['subcategoryID'] = [];
          ...
          print('[MART FIRESTORE] ❌ Error parsing Firestore item document ${doc.id}: $e');
          return null;
        }
      }).whereType<MartItemModel>().toList();
      ...
      if (searchQuery != null && searchQuery.isNotEmpty) {
        ...
        print('[MART FIRESTORE] 🔍 After search filtering: ${items.length} items');
      }
      
      print('[MART FIRESTORE] ✅ Successfully parsed ${items.length} items from Firestore');
      
      return items;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching items by category from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartVendorModel>> getMartVendors({
    ...
    try {
      ...
      print('[MART FIRESTORE] 🏪 Firestore query completed, found ${querySnapshot.docs.length} vendors');
      ...
      final vendors = querySnapshot.docs.map((doc) {
        try {
          final data = doc.data();
          if (data == null) {
            print('[MART FIRESTORE] ⚠️ Document data is null for ${doc.id}');
          ...
          print('[MART FIRESTORE] ❌ Error parsing Firestore vendor document ${doc.id}: $e');
          return null;
        }
      }).whereType<MartVendorModel>().toList();
      
      // Apply search filter on client side if needed
      if (search != null && search.isNotEmpty) {
        final searchLower = search.toLowerCase();
        vendors.removeWhere((vendor) => 
          vendor.name != null && !vendor.name!.toLowerCase().contains(searchLower)
        );
        print('[MART FIRESTORE] 🔍 After search filtering: ${vendors.length} vendors');
      }
      
      print('[MART FIRESTORE] ✅ Successfully parsed ${vendors.length} vendors from Firestore');
  ...
  Future<List<MartCategoryModel>> getPublishedCategories({String? martId}) async {
    try {
      print('[MART FIRESTORE] 📂 Fetching published categories from Firestore...');
      
      // Query Firestore for published categories
      Query query = _firestore.collection('mart_categories').where('publish', isEqualTo: true);
      ...
      print('[MART FIRESTORE] 📂 Firestore query completed, found ${querySnapshot.docs.length} published categories');
      ...
      print('[MART FIRESTORE] ✅ Successfully parsed ${categories.length} published categories from Firestore');
      
      return categories;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching published categories from Firestore: $e');
      return [];
    }
  }

  /// Get featured categories from Firestore
  Future<List<MartCategoryModel>> getFeaturedCategories({String? martId}) async {
    try {
      print('[MART FIRESTORE] ⭐ Fetching featured categories from Firestore...');
      
      // Query Firestore for featured categories
      Query query = _firestore.collection('mart_categories')
          .where('publish', isEqualTo: true)
          .where('isFeature', isEqualTo: true);
      ...
      print('[MART FIRESTORE] ⭐ Firestore query completed, found ${querySnapshot.docs.length} featured categories');
      ...
      print('[MART FIRESTORE] ✅ Successfully parsed ${categories.length} featured categories from Firestore');
      
      return categories;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching featured categories from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartItemModel>> getItemsByVendor({
    ...
    try {
      ...
      Query query = _firestore.collection('mart_items')
          .where('publish', isEqualTo: true)
      ...
      print('[MART FIRESTORE] 🏪 Firestore query completed, found ${querySnapshot.docs.length} items for vendor');
      ...
      if (categoryId != null && categoryId.isNotEmpty) {
        ...
        print('[MART FIRESTORE] 🔍 After category filtering: ${items.length} items');
      }
      
      print('[MART FIRESTORE] ✅ Successfully parsed ${items.length} items for vendor from Firestore');
      
      return items;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching items by vendor from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartItemModel>> getItemsByCategoryOnly({
    ...
    try {
      ...
      print('[MART FIRESTORE] 📂 Firestore query completed, found ${querySnapshot.docs.length} items for category');
      ...
      print('[MART FIRESTORE] ✅ Successfully parsed ${items.length} items for category from Firestore');
      
      return items;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching items by category only from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<String>> getUniqueSections() async {
    try {
      print('[MART FIRESTORE] 📂 Fetching unique sections from mart_items (OPTIMIZED)...');
      ...
      final querySnapshot = await _firestore
          ...
          .timeout(
            ...
            onTimeout: () {
              ...
              throw TimeoutException('Sections query timeout', const Duration(seconds: 5));
            },
          );
      ...
      print('[MART FIRESTORE] ✅ Found ${sections.length} unique sections: $sections');
  ...
  Future<List<MartItemModel>> getItemsBySection({
    ...
    try {
      ...
      print('[MART FIRESTORE] 🛍️ Firestore query completed, found ${querySnapshot.docs.length} items for section "$section"');
      ...
      final items = querySnapshot.docs.map((doc) {
        try {
          ...
          if (itemData['proteins'] is String) {
            itemData['proteins'] = double.tryParse(itemData['proteins']) ?? 0.0;
          }
          if (itemData['fats'] is String) {
          ...
          if (itemData['options_count'] is String) {
            itemData['options_count'] = int.tryParse(itemData['options_count']) ?? 0;
          }
          
          // Handle boolean fields that might be null
          if (itemData['has_options'] == null) itemData['has_options'] = false;
          if (itemData['isAvailable'] == null) itemData['isAvailable'] = true;
          if (itemData['isBestSeller'] == null) itemData['isBestSeller'] = false;
          if (itemData['isFeature'] == null) itemData['isFeature'] = false;
          if (itemData['isNew'] == null) itemData['isNew'] = false;
          if (itemData['isSeasonal'] == null) itemData['isSeasonal'] = false;
          if (itemData['isSpotlight'] == null) itemData['isSpotlight'] = false;
          if (itemData['isStealOfMoment'] == null) itemData['isStealOfMoment'] = false;
          if (itemData['isTrending'] == null) itemData['isTrending'] = false;
          if (itemData['veg'] == null) itemData['veg'] = true;
          if (itemData['nonveg'] == null) itemData['nonveg'] = false;
          if (itemData['takeawayOption'] == null) itemData['takeawayOption'] = false;
      ...
      print('[MART FIRESTORE] ✅ Successfully parsed ${items.length} items for section "$section"');
      return items;
    } catch (e) {
  ...
  Future<List<MartItemModel>> getMartItems({
    ...
    try {
      ...
      print('[MART FIRESTORE] ✅ Successfully parsed ${items.length} mart items from Firestore');
  ...
  Future<List<MartCategoryModel>> getCategoriesWithSubcategories({int limit = 50}) async {
    try {
      print('[MART FIRESTORE] 📂 Fetching categories with subcategories from Firestore...');
      ...
      print('[MART FIRESTORE] ✅ Successfully parsed ${categories.length} categories with subcategories from Firestore');
      
      return categories;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching categories with subcategories from Firestore: $e');
      return [];
    }
  }
  ...
  Future<List<MartCategoryModel>> getFilteredCategories({
    ...
    try {
      print('[MART FIRESTORE] 🔍 Fetching filtered categories from Firestore...');
      ...
      if (search != null && search.isNotEmpty) {
        ...
        print('[MART FIRESTORE] 🔍 After search filtering: ${categories.length} categories');
      }
      
      // Sort categories by category_order
      categories.sort((a, b) => (a.categoryOrder ?? 0).compareTo(b.categoryOrder ?? 0));
      
      print('[MART FIRESTORE] ✅ Successfully parsed ${categories.length} filtered categories from Firestore');
      
      return categories;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error fetching filtered categories from Firestore: $e');
      return [];
    }
  }
  ...
  Future<MartCategoryModel?> getCategoryById(String categoryId) async {
    try {
      ...
      // Ensure data is a Map<String, dynamic>
      if (data is! Map<String, dynamic>) {
        print('[MART FIRESTORE] ⚠️ Category data is not a Map for $categoryId, type: ${data.runtimeType}');
        return null;
      }
      ...
      print('[MART FIRESTORE] ✅ Successfully retrieved category: ${category.title}');
  ...
  Future<MartVendorModel?> getMartVendorDetails(String vendorId) async {
    try {
      ...
      // Ensure data is a Map<String, dynamic>
      if (data is! Map<String, dynamic>) {
        print('[MART FIRESTORE] ⚠️ Vendor data is not a Map for $vendorId, type: ${data.runtimeType}');
        return null;
      }
  ...
  Future<MartCategoryModel?> getCategoryDetails(String categoryId) async {
    try {
      ...
      final category = MartCategoryModel.fromJson(categoryData);
      print('[MART FIRESTORE] ✅ Successfully retrieved category details: ${category.title}');
      
      return category;
    } catch (e) {
      print('[MART FIRESTORE] ❌ Error getting category details $categoryId: $e');
      return null;
    }
  }
  ...
  Future<MartItemModel?> getItemById(String itemId) async {
    try {
      ...
      print('[MART FIRESTORE] 🔍 Document ID not found, searching by id field: $itemId');
      final querySnapshot = await _firestore
          .collection('mart_items')
      ...
      // Ensure data is a Map<String, dynamic>
      if (data is! Map<String, dynamic>) {
        print('[MART FIRESTORE] ⚠️ Item data is not a Map for $itemId, type: ${data.runtimeType}');
        return null;
      }
  ...
  Future<int> getItemCountForCategory(String categoryId) async {
    try {
      ...
      print('[MART FIRESTORE] ❌ Error getting item count for category $categoryId: $e');
      return 0;
    }
  }
  ...
  Stream<List<MartItemModel>> streamSimilarProducts({
    ...
    try {
      print('[MART FIRESTORE] 📡 Starting stream for similar products - category: $categoryId');
      if (subcategoryId != null) {
      ...
      // Apply limit
      query = query.limit(limit + (excludeProductId != null ? 1 : 0)); // Get one extra in case we need to exclude
      
      // Return stream that converts Firestore snapshots to MartItemModel list
      return query.snapshots().map((snapshot) {
        print('[MART FIRESTORE] 📡 Stream update: ${snapshot.docs.length} documents received');
        
        if (snapshot.docs.isEmpty) {
          print('[MART FIRESTORE] ⚠️ No items found in stream for category: $categoryId');
        ...
        print('[MART FIRESTORE] 📡 Stream returning ${finalItems.length} similar products');
        return finalItems;
      }).handleError((error) {
      ...
      print('[MART FIRESTORE] ❌ Error creating stream for similar products: $e');
      ...
      return query.snapshots().map((snapshot) {
        ...
        for (final doc in snapshot.docs) {
          try {
            ...
            print('[MART FIRESTORE] 📡 Processing document ${doc.id}: ${itemData['name']}');
        ...
        print('[MART FIRESTORE] 📡 Stream returning ${finalItems.length} products');
        print('[MART FIRESTORE] 📡 Product names: ${finalItems.map((item) => item.name).toList()}');
        return finalItems;
      }).handleError((error) {
  ...
  Stream<List<MartItemModel>> streamProductDeals({
    ...
    try {
      ...
      return query.snapshots().map((snapshot) {
        print('[MART FIRESTORE] 📡 Product Deals stream: ${snapshot.docs.length} documents');
  ...
  Stream<List<MartItemModel>> streamHairCareProducts({
    ...
    try {
      ...
      return query.snapshots().map((snapshot) {
        print('[MART FIRESTORE] 📡 Hair Care stream: ${snapshot.docs.length} documents');
        return _parseSnapshotToMartItems(snapshot);
      }).handleError((error) {
  ...
  Stream<List<MartItemModel>> streamChocolateProducts({
    ...
    try {
      ...
      Query query = _firestore.collection('mart_items')
          .where('publish', isEqualTo: true)
          .where('isAvailable', isEqualTo: true)
          .where('categoryTitle', isEqualTo: 'Chocolates') // Chocolates category
          .limit(limit);
      
      return query.snapshots().map((snapshot) {
        print('[MART FIRESTORE] 📡 Chocolates stream: ${snapshot.docs.length} documents');
        return _parseSnapshotToMartItems(snapshot);
      }).handleError((error) {
  ...
  Stream<List<MartItemModel>> streamPlaytimeProducts({
    ...
    try {
      ...
      return query.snapshots().map((snapshot) {
        print('[MART FIRESTORE] 📡 Playtime stream: ${snapshot.docs.length} documents');
        return _parseSnapshotToMartItems(snapshot);
      }).handleError((error) {
  ...
  Stream<List<MartItemModel>> streamBabyCareProducts({
    ...
    try {
      ...
      return query.snapshots().map((snapshot) {
        print('[MART FIRESTORE] 📡 Baby Care stream: ${snapshot.docs.length} documents');
        return _parseSnapshotToMartItems(snapshot);
      }).handleError((error) {
  ...
  Stream<List<MartItemModel>> streamLocalGroceryProducts({
    ...
    try {
      ...
      Query query = _firestore.collection('mart_items')
          .where('publish', isEqualTo: true)
          .where('isAvailable', isEqualTo: true)
          .where('categoryTitle', isEqualTo: 'Local Grocery') // Local grocery category
          .limit(limit);
      
      return query.snapshots().map((snapshot) {
        print('[MART FIRESTORE] 📡 Local Grocery stream: ${snapshot.docs.length} documents');
        return _parseSnapshotToMartItems(snapshot);
      }).handleError((error) {
  ...
  Stream<List<MartBannerModel>> streamBannersByPosition(String position, {int limit = 10}) {
    try {
      ...
      return FirebaseFirestore.instance
          ...
          .snapshots()
          .map((snapshot) {
        print('DEBUG: Banner snapshot received with ${snapshot.docs.length} documents');
        ...
        for (var doc in snapshot.docs) {
          try {
            ...
            banners.add(banner);
            print('DEBUG: Added banner: ${banner.title} (order: ${banner.setOrder})');
          } catch (e) {
            print('ERROR: Failed to parse banner document ${doc.id}: $e');
          }
        }
        
        print('DEBUG: Returning ${banners.length} banners for position: $position');
        return banners;
      });
    } catch (e) {
  ...
  Stream<List<MartBannerModel>> streamAllBanners({int limit = 20}) {
    try {
      ...
      return FirebaseFirestore.instance
          ...
          .snapshots()
          .map((snapshot) {
        print('DEBUG: All banners snapshot received with ${snapshot.docs.length} documents');
        ...
        for (var doc in snapshot.docs) {
          try {
            final banner = MartBannerModel.fromMap(doc.data(), doc.id);
            banners.add(banner);
            print('DEBUG: Added banner: ${banner.title} (position: ${banner.position}, order: ${banner.setOrder})');
          } catch (e) {
            print('ERROR: Failed to parse banner document ${doc.id}: $e');
          }
        }
  ...
  Future<List<MartBannerModel>> getBannersByPosition(String position, {int limit = 10}) async {
    try {
      ...
      print('DEBUG: Banner fetch completed with ${snapshot.docs.length} documents');
  ...
  Stream<List<MartItemModel>> streamItemsByBrand(String brandID) {
    try {
      ...
      return _firestore
          ...
          .snapshots()
          .map((snapshot) {
        print('[MART FIRESTORE] 📦 Brand items snapshot received with ${snapshot.docs.length} documents');
        ...
        for (var doc in snapshot.docs) {
          try {
            ...
            print('[MART FIRESTORE] ❌ Error parsing brand item document ${doc.id}: $e');
          }
        }
        
        print('[MART FIRESTORE] 📦 Returning ${items.length} items for brand: $brandID');
        return items;
      });
    } catch (e) {
