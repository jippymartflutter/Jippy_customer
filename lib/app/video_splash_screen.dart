...
class _VideoSplashScreenState extends State<VideoSplashScreen> {
  ...
  void _initializeLogo() async {
    try {
      ...
      _navigationTimer = Timer(const Duration(milliseconds: 300), () {
        if (mounted) {
          developer.log('VideoSplashScreen: Splash duration completed, navigating to main app');
          _navigateToMainApp();
        }
      });
  ...
  void _navigateToMainApp() async {
    ...
    try {
      ...
      developer.log('[VIDEO_SPLASH] api_token from secure storage: ${apiToken != null ? "EXISTS (${apiToken.length} chars)" : "NULL"}');

      // 2. Check Firebase user
      User? firebaseUser = FirebaseAuth.instance.currentUser;
      developer.log('[VIDEO_SPLASH] Firebase currentUser: ${firebaseUser?.uid ?? "NULL"}');
      developer.log('[VIDEO_SPLASH] Firebase user email: ${firebaseUser?.email ?? "NULL"}');
      developer.log('[VIDEO_SPLASH] Firebase user phone: ${firebaseUser?.phoneNumber ?? "NULL"}');
      developer.log('[VIDEO_SPLASH] Firebase user isAnonymous: ${firebaseUser?.isAnonymous ?? "NULL"}');

      // If no API token or no Firebase user, go to phone number login
      if (apiToken == null || apiToken.isEmpty || firebaseUser == null) {
        developer.log('[VIDEO_SPLASH] No API token or Firebase user. Checking for app updates before login...');
        
        // DEBUG MODE: Additional logging for debugging logout issues
        if (kDebugMode) {
          developer.log('[VIDEO_SPLASH] DEBUG MODE - Authentication state:');
          developer.log('[VIDEO_SPLASH] DEBUG - API Token: ${apiToken != null ? "EXISTS" : "NULL"}');
          developer.log('[VIDEO_SPLASH] DEBUG - Firebase User: ${firebaseUser != null ? "EXISTS" : "NULL"}');
          developer.log('[VIDEO_SPLASH] DEBUG - Firebase Auth State: ${FirebaseAuth.instance.authStateChanges().first}');
        }
        
        // **FIXED: Check for app updates asynchronously to prevent blocking**
        developer.log('[VIDEO_SPLASH] Checking for app updates asynchronously...');
        
        // Navigate immediately and check updates in background
        developer.log('[VIDEO_SPLASH] Navigating to PhoneNumberScreen immediately.');
        Get.offAll(() => const PhoneNumberScreen(), transition: Transition.fadeIn, duration: const Duration(milliseconds: 800));
      ...
      if (apiToken != null && apiToken.isNotEmpty && firebaseUser != null) {
        developer.log('[VIDEO_SPLASH] All tokens present, navigating immediately...');
        
        // **FIXED: Navigate immediately and verify user profile in background**
        developer.log('[VIDEO_SPLASH] Navigating to DashBoardScreen immediately.');
        Get.offAll(() => const DashBoardScreen(), transition: Transition.fadeIn, duration: const Duration(milliseconds: 800));
        ...
        try {
          ...
          print('🔗 [VIDEO_SPLASH] Processing pending deep link after login...');
        } catch (e) {
    ...
    Future.microtask(() async {
      try {
        ...
        bool updateRequired = await AppUpdateService.checkForUpdate().timeout(
          ...
          onTimeout: () {
            developer.log('[VIDEO_SPLASH] Background: Update check timed out, continuing...');
            return false;
          },
        );
        
        if (updateRequired) {
          developer.log('[VIDEO_SPLASH] Background: Update required, will show update dialog when ready.');
          // The AppUpdateService will handle showing the update dialog
        } else {
          developer.log('[VIDEO_SPLASH] Background: No update required.');
        }
      } catch (e) {
        developer.log('[VIDEO_SPLASH] Background: Error checking for updates: $e');
    ...
    ANRPrevention.executeWithANRPrevention(
      ...
      () async {
        try {
          ...
          UserModel? userModel = await FireStoreUtils.getUserProfile(userId).timeout(
            const Duration(seconds: 3), // Reduced from 15s to 3s
            onTimeout: () {
              developer.log('[VIDEO_SPLASH] Background: User profile check timed out after 3s, continuing...');
              return null;
            },
          );
          
          if (userModel != null && userModel.active == true) {
            developer.log('[VIDEO_SPLASH] Background: User profile verified and active.');
          } else {
            developer.log('[VIDEO_SPLASH] Background: User profile missing or inactive. Will handle in background.');
            // Could show a toast or handle this gracefully without blocking the user
          }
        } catch (e) {
          developer.log('[VIDEO_SPLASH] Background: Error verifying user profile: $e');
