...
class MartBrandProductsScreen extends StatefulWidget {
  ...
  @override
  State<MartBrandProductsScreen> createState() => _MartBrandProductsScreenState();
}
...
class _MartBrandProductsScreenState extends State<MartBrandProductsScreen> {
  ...
  final CategoryDetailController _categoryController = Get.find<CategoryDetailController>();
  ...
  Future<void> _fetchBrandData() async {
    try {
      final doc = await _firestore.collection('brands').doc(widget.brandID).get();
      if (doc.exists) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: const Color(0xFF20B2AA), // Teal color like search section
        elevation: 0,
        leading: Container(
              ...
              Expanded(
                child: Align(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      color: Colors.transparent, // Remove white background layer
