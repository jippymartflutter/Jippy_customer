...
class _MartProductDetailsScreenState extends State<MartProductDetailsScreen> with WidgetsBindingObserver {
  int quantity = 1;
  ...
  Future<void> _fetchBrandData() async {
    if (widget.product.brandID != null && widget.product.brandID!.isNotEmpty) {
      try {
        final doc = await _firestore.collection('brands').doc(widget.product.brandID).get();
        if (doc.exists) {
          setState(() {
  ...
  void _checkCartStatus() {
    try {
      print('[CART STATUS] Checking cart status for product: ${widget.product.id}');
      ...
      print('[CART STATUS] Found cart item: ${foundCartItem.id}, quantity: ${foundCartItem.quantity}');
      
      if (foundCartItem.id != null && foundCartItem.quantity != null && foundCartItem.quantity! > 0) {
        print('[CART STATUS] Product is in cart with quantity: ${foundCartItem.quantity}');
        setState(() {
          isInCart = true;
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Brand Information
                  if (widget.product.brandTitle != null && widget.product.brandTitle!.isNotEmpty) ...[
                    const SizedBox(height: 8),
                    GestureDetector(
                      onTap: () {
                        // Navigate to brand products page
                        _navigateToBrandProducts(widget.product.brandID, widget.product.brandTitle!);
                      },
                      child: Container(
                  ...
                  Column(
                    children: [
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            Icon(Icons.flash_on, color: Colors.grey[600], size: 20),
                            const SizedBox(width: 8),
                            Expanded(
                              child: Text(
                                'Fast Delivery',
                  ...
                  // Product Information
                  if (widget.product.brand != null || widget.product.weight != null || widget.product.expiryDate != null) ...[
                    const Text(
                      'Product Information',
                      style: TextStyle(
                        fontSize: 18,
        ...
        onPressed: () async {
          try {
            ...
            final cartProduct = CartProductModel(
              ...
              discountPrice: widget.product.disPrice?.toString() ?? widget.product.price?.toString() ?? '0',
            ...
            print('[CART] Product details - Adding to cart: ${cartProduct.name} (ID: ${cartProduct.id})');
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  ...
                  action: SnackBarAction(
                    ...
                    onPressed: () {
                      Get.to(() => const CartScreen(hideBackButton: false, source: 'mart', isFromMartNavigation: false),
  ...
  Widget _buildCartActions() {
    ...
    final totalCartItems = cartItem.fold(0, (sum, item) => sum + (item.quantity ?? 0));
  ...
  Widget _buildSimilarProducts() {
    ...
    print('[SIMILAR PRODUCTS] Current product categoryID: ${widget.product.categoryID}');
    print('[SIMILAR PRODUCTS] Current product subcategoryID: ${widget.product.subcategoryID}');
    
    // Check if we have the required categoryID
    if (widget.product.categoryID == null || widget.product.categoryID!.isEmpty) {
      print('[SIMILAR PRODUCTS] No categoryID available, cannot load similar products');
      return Container(
        padding: const EdgeInsets.all(20),
        decoration: BoxDecoration(
    ...
    return StreamBuilder<List<MartItemModel>>(
      ...
      builder: (context, snapshot) {
        print('[SIMILAR PRODUCTS] Stream snapshot state: ${snapshot.connectionState}');
        print('[SIMILAR PRODUCTS] Stream has data: ${snapshot.hasData}');
        print('[SIMILAR PRODUCTS] Stream has error: ${snapshot.hasError}');
        if (snapshot.hasData) {
          print('[SIMILAR PRODUCTS] Stream data length: ${snapshot.data?.length ?? 0}');
          print('[SIMILAR PRODUCTS] Stream data: ${snapshot.data?.map((item) => item.name).toList()}');
        }
        ...
        if (snapshot.hasData && snapshot.data!.isNotEmpty) {
          ...
          print('[SIMILAR PRODUCTS] Stream returned ${similarProducts.length} similar products');
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        style: ElevatedButton.styleFrom(
                          backgroundColor: const Color(0xFFE91E63),
                          foregroundColor: Colors.white,
                          padding: const EdgeInsets.symmetric(vertical: 4), // Smaller padding
                          shape: RoundedRectangleBorder(
