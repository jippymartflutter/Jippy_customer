...
class MartHomeScreen extends StatelessWidget {
                  ...
                  RefreshIndicator(
                    ...
                    child: CustomScrollView(
                      slivers: [
                        // Modern Header Card
                        SliverToBoxAdapter(
                          child: ModernMartHeaderCard(screenWidth: screenWidth),
                        ),
                        ...
                        // Search Widget
                        SliverToBoxAdapter(
                          child: ModernSearchWidget(),
                        ),

                        // Quick Categories Section
                        SliverToBoxAdapter(
                          child: ModernQuickCategories(screenWidth: screenWidth),
                        ),

                        // Featured Products
                        SliverToBoxAdapter(
                          child: ModernFeaturedProducts(screenWidth: screenWidth),
                        ),
                        ...
                        // Trending Deals
                        SliverToBoxAdapter(
                          child: ModernTrendingDeals(screenWidth: screenWidth),
                        ),
                        ...
                        SliverToBoxAdapter(
                          child: ModernProductDealsSection(screenWidth: screenWidth),
                        ),

                        // Dynamic Sections
                        SliverToBoxAdapter(
                          child: ModernDynamicSections(screenWidth: screenWidth),
                        ),
  ...
  void _launchWhatsApp() async {
    ...
    const String message = 'Hello! I need help with my JippyMart order.';
...
// Modern Header Card
class ModernMartHeaderCard extends StatefulWidget {
  final double screenWidth;

  const ModernMartHeaderCard({super.key, required this.screenWidth});

  @override
  State<ModernMartHeaderCard> createState() => _ModernMartHeaderCardState();
}

class _ModernMartHeaderCardState extends State<ModernMartHeaderCard> {
  bool isMartSelected = true;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: [
            const Color(0xFF00998a).withOpacity(0.9),
  ...
  String _getUserName() {
    ...
    return userModel?.firstName?.trim() ?? 'Guest';
  }
}

// Modern Search Widget
class ModernSearchWidget extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: const BoxDecoration(
        color: Color(0xFFFAF9EE),
      ),
      child: Column(
        children: [
          // Modern Search Bar
...
class ModernQuickCategories extends StatelessWidget {
  final double screenWidth;

  const ModernQuickCategories({super.key, required this.screenWidth});
      ...
      builder: (controller) {
        ...
        return Container(
          ...
          child: Row(
            ...
            children: [
              _buildCategoryItem(controller.featuredCategories[0], 0),
              _buildCategoryItem(
                  controller.featuredCategories.length > 1
                      ? controller.featuredCategories[1]
                      : null,
                  1),
              _buildCategoryItem(
                  controller.featuredCategories.length > 2
                      ? controller.featuredCategories[2]
                      : null,
                  2),
            ],
          ),
  ...
  Widget _buildCategoryItem(MartCategoryModel? category, int index) {
    ...
    if (category == null) {
      return _buildCategoryPlaceholder(colors[index]);
    }
  ...
  Widget _buildCategoryPlaceholder(List<Color> colors) {
    return Column(
      children: [
        Container(
          width: 64,
...
class ModernFeaturedProducts extends StatelessWidget {
  final double screenWidth;

  const ModernFeaturedProducts({super.key, required this.screenWidth});
  ...
  Widget _buildProductsGrid(MartController controller) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final screenWidth = constraints.maxWidth;
        final isTablet = screenWidth > 600;
        final cardWidth = isTablet ? 200.0 : 180.0;
...
// Modern Trending Deals
class ModernTrendingDeals extends StatelessWidget {
  final double screenWidth;

  const ModernTrendingDeals({super.key, required this.screenWidth});
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        ...
        print('[MART HOME] Controller not found: $e');
      }
    });
          ...
          GetX<MartController>(
            builder: (controller) {
              if (controller.isTrendingLoading.value) {
                return _buildTrendingLoading();
              }

              if (controller.filteredTrendingItems.isEmpty) {
                return _buildTrendingEmptyState();
              }

              return _buildTrendingProducts(controller);
            },
          ),
        ],
      ),
  ...
  Widget _buildTrendingProducts(MartController controller) {
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      padding: const EdgeInsets.only(left: 20, right: 20),
      child: Row(
...
// Modern Product Deals Section
class ModernProductDealsSection extends StatelessWidget {
  final double screenWidth;

  const ModernProductDealsSection({super.key, required this.screenWidth});
            ...
            child: StreamBuilder<List<MartItemModel>>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return _buildDealsLoading();
                }

                if (snapshot.hasError || snapshot.data == null || snapshot.data!.isEmpty) {
                  return _buildDealsEmptyState();
                }

                return _buildDealsList(snapshot.data!);
              },
            ),
          ),
        ],
  ...
  Widget _buildDealsList(List<MartItemModel> products) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        return Container(
          margin: const EdgeInsets.only(right: 16),
          child: _martItemToPlaytimeCard(products[index], screenWidth),
        );
      },
    );
  }
...
class ModernDynamicSections extends StatefulWidget {
  final double screenWidth;

  const ModernDynamicSections({super.key, required this.screenWidth});

  @override
  State<ModernDynamicSections> createState() => _ModernDynamicSectionsState();
}

class _ModernDynamicSectionsState extends State<ModernDynamicSections> {
