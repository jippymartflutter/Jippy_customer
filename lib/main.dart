...
void main() async {
  ...
  print('🔗 [MAIN] GlobalDeeplinkHandler initialized and registered successfully');
  ...
  try {
    ...
    print('⚠️ [MAIN] Firebase initialization failed, continuing without monitoring systems');
  }
...
void _initializeHeavyServicesInBackground() {
  ...
  Future.microtask(() async {
    try {
      ...
      await Future.wait([
        Get.putAsync(() => ApiService().init()).timeout(const Duration(seconds: 5)),
        Get.putAsync(() => MartFirestoreService().init()).timeout(const Duration(seconds: 5)),
      ...
      await Future.wait([
        PendingDeepLinkHandler.checkPendingDeepLinks().timeout(const Duration(seconds: 3)),
        MobileDeepLinkService().initialize().timeout(const Duration(seconds: 3)),
      ]);
...
void _initializeDeepLinkServicesInBackground() {
  Future.microtask(() async {
    try {
      ...
      await FinalDeepLinkService().init(GlobalDeeplinkHandler.navigatorKey)
          .timeout(const Duration(seconds: 5));
      print('🔗 [MAIN] ✅ Background: Final Deep Link Service initialized successfully');
      log('🔗 [MAIN] Background: Final Deep Link Service initialized successfully');
    } catch (e) {
      print('🔗 [MAIN] ❌ Background: Deep Link Service initialization failed: $e');
...
void _initializeSmartLookInBackground() {
  Future.microtask(() async {
    ...
    try {
      ...
      await smartlookService.initialize(
      ...
      if (smartlookService.isInitialized) {
        ...
        print('[SMARTLOOK] ✅ Initialized successfully in background with crash prevention');
      }
    } catch (e) {
      ...
      try {
        ...
        await smartlookService.preventSessionRecordingStorageCrash();
        await smartlookService.forceReinitialize(SmartlookConfig.projectKey, region: SmartlookConfig.region);
        if (smartlookService.isInitialized) {
    ...
    return ChangeNotifierProvider(
      ...
      child: Consumer<DarkThemeProvider>(
        builder: (context, value, child) {
          ...
          // ✅ CRITICAL: Wrap with Smartlook only if initialization was successful
          try {
            return SmartlookRecordingWidget(
              child: appWidget,
            );
          } catch (e) {
            print('[SMARTLOOK] Error wrapping with SmartlookRecordingWidget: $e');
