...
class ANRMonitor {
  ...
  static void startMonitoring() {
    ...
    try {
      FirebaseCrashlytics.instance.log('ANR_MONITOR: Started monitoring for ANR detection');
    } catch (e) {
  ...
  static void _reportANRWarning(Duration blockedTime) {
    _anrWarnings++;
    final message = 'ANR_WARNING: UI blocked for ${blockedTime.inMilliseconds}ms';
    log(message);
    ...
    try {
      ...
      FirebaseCrashlytics.instance.recordError(
        Exception('ANR Warning: UI blocked for ${blockedTime.inMilliseconds}ms'),
        StackTrace.current,
  ...
  static Map<String, dynamic> getMonitoringStats() {
    return {
      ...
      'anrWarningRate': _totalChecks > 0 ? (_anrWarnings / _totalChecks * 100).toStringAsFixed(2) : '0.00',
...
class PerformanceMetrics {
  ...
  static Map<String, dynamic> getPerformanceReport() {
    ...
    for (final entry in _operationMetrics.entries) {
      ...
      if (durations.isNotEmpty) {
        avgDurations[entry.key] = durations
    ...
    return {
      ...
      'slowOperationPercentage': _totalOperations > 0 ? (_slowOperations / _totalOperations * 100).toStringAsFixed(2) : '0.00',
      'timeoutPercentage': _totalOperations > 0 ? (_timeoutOperations / _totalOperations * 100).toStringAsFixed(2) : '0.00',
  ...
  static void logMetricsToFirebase() {
    ...
    try {
      ...
      FirebaseCrashlytics.instance.log('Total Operations: ${report['totalOperations']}');
      FirebaseCrashlytics.instance.log('Slow Operations: ${report['slowOperations']}');
      FirebaseCrashlytics.instance.log('Slow Operation %: ${report['slowOperationPercentage']}%');
      FirebaseCrashlytics.instance.log('Timeout Operations: ${report['timeoutOperations']}');
      FirebaseCrashlytics.instance.log('Timeout %: ${report['timeoutPercentage']}%');
    } catch (e) {
...
class MemoryMonitor {
  ...
  static void startMemoryMonitoring() {
    ...
    // Only log to Firebase if it's available
    try {
      FirebaseCrashlytics.instance.log('MEMORY_MONITOR: Started memory monitoring');
    } catch (e) {
...
class ANRStatusLogger {
  ...
  static void logOperationFailure(String operationName, String error) {
    ...
    try {
      FirebaseCrashlytics.instance.log(
        'OPERATION_FAILURE: $operationName failed - $error'
      );
    } catch (e) {
  ...
  static void logANRPreventionTrigger(String operationName) {
    ...
    try {
      FirebaseCrashlytics.instance.log(
        'ANR_PREVENTION_TRIGGERED: $operationName moved to background'
      );
    } catch (e) {
